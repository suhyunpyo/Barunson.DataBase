IF OBJECT_ID (N'dbo.SP_S_VW_USER_INFO_LOGIN', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_S_VW_USER_INFO_LOGIN
GO
...
SELECT * FROM BARUNSON.DBO.VW_USER    WHERE UID = 'netbluewtest' AND PWD = '123456'

EXEC barunson.DBO.SP_S_VW_USER_INFO_LOGIN   's4guest', 'ysmguest1', ''

EXEC SP_SELECT_BARUNSON.DBO.VW_USER   _FOR_LOGIN 's4guest', 'ysmguest', 'nlvcuiejG4Q4LULtdhCIWYCJWQH/OwgACtpCsWk7btnOg6SLDCevGALcc1oChqyYAhPzuDrvKwLnstvkb4HbLw=='

EXEC SP_SELECT_BARUNSON.DBO.VW_USER   _FOR_LOGIN '', '', 'MC0GCCqGSIb3DQIJAyEAItjeCII7f+cjTKLkoaAz1mlAM+CsYxeUFAONtU2u8UM='

EXEC SP_SELECT_BARUNSON.DBO.VW_USER   _FOR_LOGIN '', '', 'MC0GCCqGSIb3DQIJAyEAFY85yisHhwjR+ZpVUTIJCZ0dsjJ27JuA5U0JBE7Ybmw='
...
SELECT  *
FROM    BARUNSON.DBO.VW_USER   
WHERE   UID = ''
AND     PWD = ''

EXEC SP_SELECT_BARUNSON.DBO.VW_USER   _FOR_LOGIN 'sharniel', '22642057', ''
원본 - bar_shop1.dbo.SP_SELECT_VW_USER_INFO_FOR_LOGIN 
use barunson
SP_S_VW_USER_INFO_LOGIN  'vyrudaks12', 'pkm1103!@'
*/
CREATE PROCEDURE [dbo].[SP_S_VW_USER_INFO_LOGIN]
...
AS
 ...
 BEGIN
    ...
    IF (@USER_ID <> '' AND @PASSWORD <> '') OR @DUPINFO <> '' BEGIN
    ...
    /* 결과값 셋팅 */
    SET @RESULT_CODE =  (
                            CASE WHEN @USER_COUNT = 1 THEN '0000'    -- 정상
	...
	-- 사용자의 입력값과 결과값을 따로 저장합니다.
	-- CS 대응할 때만 잠깐 사용하는 것으로 개선
	IF 1 = 2
	BEGIN
		INSERT INTO BARUNSON.DBO.TB_Value_Entered_Login (USER_ID, PASSWORD, DUPINFO, RESULT_CODE) VALUES (@USER_ID, @PASSWORD, @DUPINFO, @RESULT_CODE)
	END
 ...
 --       --,   CASE WHEN NA_VUI.UID IS NULL THEN 'Y' ELSE 'N' END AS AVAILABLE_ID_YORN
    ...
    WHERE   1 = 1
    --AND     VUI.USE_YORN = 'Y'
    AND VUI.DUPINFO =  @DUPINFO AND
		USER_ID = @USER_ID AND 
		(PWDCOMPARE(@PASSWORD, TRY_CONVERT(VARBINARY(200), PWD, 1)) = 1)
	
	--AND	
	--VUI.SITE_DIV_NAME <> '기타'
	ORDER BY 
            (CASE WHEN VUI.USER_ID = @USER_ID AND PWDCOMPARE(@PASSWORD, TRY_CONVERT(VARBINARY(200), VUI.PWD, 1)) = 1 THEN 1 ELSE 99 END) ASC	-- 아이디, 패스워드가 일치하는 기준으로 오름차순
