IF OBJECT_ID (N'dbo.SP_S_SCHEDULE_ORDER_STATISTICS', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_S_SCHEDULE_ORDER_STATISTICS
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[SP_S_SCHEDULE_ORDER_STATISTICS]
	@YMD VARCHAR(8) = NULL
AS
BEGIN
	SET NOCOUNT ON;
	...
	IF @YMD is NULL BEGIN
		SET @YMD = convert(varchar, dateadd(DAY, -1, getdate()), 112)
	END

	SET @YM = left(@YMD, 6);

/* 전체현황 일별 통계 등록 */
delete from TB_Total_Statistic_Day where Date = @YMD;
...
/* 매출통계 (일별) */
delete from TB_Sales_Statistic_Day where Date = @YMD;
...
/* 결제수단 일별 */
delete from TB_Payment_Status_Day where Date = @YMD;
