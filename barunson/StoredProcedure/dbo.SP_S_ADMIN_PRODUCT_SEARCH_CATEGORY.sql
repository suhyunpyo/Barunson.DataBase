IF OBJECT_ID (N'dbo.SP_S_ADMIN_PRODUCT_SEARCH_CATEGORY', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_S_ADMIN_PRODUCT_SEARCH_CATEGORY
GO
...
GO
CREATE PROCEDURE [dbo].[SP_S_ADMIN_PRODUCT_SEARCH_CATEGORY]
/***************************************************************
작성자	:	표수현
작성일	:	2020-02-15
DESCRIPTION	:	진열관리 -> 카테고리 진열 -> 상품리스트 (대분류명,소분류명을 같이 노출시켜야되는 이슈로 우선 프로시저로 구현 
SPECIAL LOGIC	: 
SP_S_ADMIN_PRODUCT_SEARCH_CATEGORY  @CATEGORY_TYPE_CODE = 'CTC02', @PRODUCT_CATEGORY_CODE = null, 
@PRODUCT_BRAND_CODE= null, @SEARCHTXT= null, @SEARCHVIEWYN= 'Y', @CATEGORY_ID= 0
...
AS
 ...
 DECLARE @CATEGORY_ID_CONDITION1 INT = 0
  DECLARE @CATEGORY_ID_CONDITION2 INT = 0
 ...
 IF @SEARCHTXT IS NOT NULL AND @SEARCHTXT <> '' BEGIN 
	...
	WHERE A.PRODUCT_CATEGORY_CODE = (CASE WHEN @CATEGORY_CODE_CONDITION1 = 1 THEN @PRODUCT_CATEGORY_CODE ELSE A.PRODUCT_CATEGORY_CODE END) AND
		  ...
		  (
			E.CATEGORY_ID = (CASE WHEN @CATEGORY_ID_CONDITION2 = 1 THEN @CATEGORY_ID2 ELSE E.CATEGORY_ID END) AND
			ISNULL(E.PARENT_CATEGORY_ID, '') = (CASE WHEN @CATEGORY_ID_CONDITION1 = 1 THEN @CATEGORY_ID1 ELSE  ISNULL(E.PARENT_CATEGORY_ID, '') END) 
		  ) AND
