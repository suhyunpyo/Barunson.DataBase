IF OBJECT_ID (N'dbo.SP_S_ADMIN_ORDER_MEMBER_NEW_TEST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_S_ADMIN_ORDER_MEMBER_NEW_TEST
GO
...
GO
CREATE PROCEDURE [dbo].[SP_S_ADMIN_ORDER_MEMBER_NEW_TEST]
/***************************************************************
작성자	:	표수현
작성일	:	2021-05-26
DESCRIPTION	:	ADMIN - 회원관리 - 주문한 회원목록 
SPECIAL LOGIC	: SP_S_ADMIN_ORDER_MEMBER_NEW_TEST '2021-09-24', '2021-10-01' , 'ALL', 'jjiminjun'
			...
			FROM (
					...
					FROM (
							SELECT	USER_ID, 
									EMAIL,
									NAME
							FROM 	BARUNSON.DBO.VW_USER_TEST(NOLOCK)
							...
							FROM 	BARUNSON.DBO.TB_ORDER AS O
									LEFT JOIN BARUNSON.DBO.VW_USER_TEST(NOLOCK) AS U
						...
						LEFT JOIN BARUNSON.DBO.VW_USER_TEST B ON BASE.USER_ID = B.USER_ID 
