IF OBJECT_ID (N'dbo.SP_S_ADMIN_ORDER_LIST_EXCEL', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_S_ADMIN_ORDER_LIST_EXCEL
GO
...
GO
CREATE PROCEDURE [dbo].[SP_S_ADMIN_ORDER_LIST_EXCEL]
/*************************************************************** 
작성자	:	표수현
작성일	:	2021-05-26
DESCRIPTION	:	ADMIN - 회원관리 - 주문한 회원목록 
SPECIAL LOGIC	: SP_S_ADMIN_ORDER_LIST_EXCEL '2021-10-01', '2021-10-31' , 'ORDER', 'ALL', 'PCC01_'
			...
			SELECT 
					 회원모초구매횟수 = (
									...
									WHERE USER_ID = T.[USER_ID] 
											AND CONVERT(VARCHAR(10), T.REGIST_DATETIME, 120) BETWEEN  CONVERT(VARCHAR(10), @START_DATE, 120) AND CONVERT(VARCHAR(10), @END_DATE, 120) 
											AND PAYMENT_STATUS_CODE = 'PSC02'
											AND T.[USER_ID] <> '' 
					...
					비회원모초구매횟수 = (
									...
									WHERE NAME = T.[NAME] AND EMAIL = T.EMAIL
											AND CONVERT(VARCHAR(10), T.REGIST_DATETIME, 120) BETWEEN  CONVERT(VARCHAR(10), @START_DATE, 120) AND  CONVERT(VARCHAR(10), @END_DATE, 120) 
											AND PAYMENT_STATUS_CODE = 'PSC02'
 ...
 END ELSE BEGIN 
	...
	FROM (  
			...
			SELECT 
					...
					회원모초구매횟수 = (
											SELECT COUNT(1) FROM TB_ORDER WHERE USER_ID = T.[USER_ID] 
											AND CONVERT(VARCHAR(10), T.PAYMENT_DATETIME, 120) BETWEEN  CONVERT(VARCHAR(10), @START_DATE, 120) AND  CONVERT(VARCHAR(10), @END_DATE, 120) 
											AND PAYMENT_STATUS_CODE = 'PSC02'
											AND T.[USER_ID]  <> '' 
											AND T.[USER_ID]  IS NOT NULL
					...
					비회원모초구매횟수 = (
									...
									SELECT COUNT(1) FROM TB_ORDER WHERE NAME = T.[NAME] AND EMAIL = T.EMAIL
									AND CONVERT(VARCHAR(10), T.PAYMENT_DATETIME, 120) BETWEEN  CONVERT(VARCHAR(10), @START_DATE, 120) AND  CONVERT(VARCHAR(10), @END_DATE, 120) 
									AND PAYMENT_STATUS_CODE = 'PSC02'
									AND (T.[USER_ID] = '' OR T.[USER_ID] IS NULL)
