IF OBJECT_ID (N'dbo.SP_T_USER_ORDER_CASEA_COUPON_PUBLISH', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_T_USER_ORDER_CASEA_COUPON_PUBLISH
GO
...
GO
CREATE PROCEDURE [dbo].[SP_T_USER_ORDER_CASEA_COUPON_PUBLISH]   
/***************************************************************  
작성자 : 표수현  
작성일 : 2021-09-17  
DESCRIPTION : 기존 바비더프몰 회원한테 쿠폰 쏘기 
			1. 종이청첩 컨펌 / 결제 완료 이후 진행중인 모든 건들 해당
			2. 모바일 청첩장 제작 유무 상관없음
...
AS    
 ...
 -- 쿠폰 발송할 회원ID리스트 
 INSERT	#WEDDING_LIST
 SELECT MIN(ORDER_SEQ), MAX(WEDDING_DAY), MEMBER_ID
 FROM (
 ...
 WHERE	CONVERT(VARCHAR(10), TB.WEDDING_DAY, 120)  >= '2021-09-27' AND	-- 예식일이 21년 9월 27일 이후
		...
		TB.MEMBER_ID NOT IN (SELECT MEMBER_ID FROM TB_CASEA_COUPON_PUBLISHED) -- ID로 중복발송 체크
 GROUP BY MEMBER_ID
 ...
 IF EXISTS (SELECT COUNT(1) FROM #WEDDING_LIST) BEGIN 
		...
		FROM (
				SELECT	A.COUPON_ID ,
					...
					FROM TB_COUPON A INNER JOIN 
						 ...
						 TB_COMMON_CODE C ON A.PUBLISH_TARGET_CODE = C.CODE  INNER JOIN 
						 TB_COMMON_CODE D ON A.DISCOUNT_METHOD_CODE = D.CODE 
					WHERE B.CODE_GROUP = 'PUBLISH_METHOD_CODE' AND 
						  ...
						  A.DISCOUNT_METHOD_CODE = 'DMC02' AND  -- 할인단위가 %
						  A.DISCOUNT_RATE = 30
			) TB 
		...
		IF EXISTS (SELECT COUNT(1) FROM #COUPON) BEGIN 
			...
			INSERT TB_CASEA_COUPON_PUBLISHED (ORDER_ID, MEMBER_ID, REG_DATE)
