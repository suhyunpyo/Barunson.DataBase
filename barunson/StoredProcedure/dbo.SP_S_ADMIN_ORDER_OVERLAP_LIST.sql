IF OBJECT_ID (N'dbo.SP_S_ADMIN_ORDER_OVERLAP_LIST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_S_ADMIN_ORDER_OVERLAP_LIST
GO
...
GO
CREATE PROCEDURE [dbo].[SP_S_ADMIN_ORDER_OVERLAP_LIST]
/***************************************************************
작성자	:	표수현
작성일	:	2021-05-26
DESCRIPTION	:	ADMIN - 회원관리 - 주문한 회원목록 
SPECIAL LOGIC	: SP_S_ADMIN_ORDER_OVERLAP_LIST '2021-10-27', '2021-11-03' , 'ORDER11','ALL','PCC01_PCC02_','PBC01_PBC02_PBC03_PBC04_','ALL',NULL
...
AS
 ...
 IF @SEARCHPERIOD = 'ORDER' BEGIN 
	...
	-- 중복구매한 회원아이디 
	SELECT * INTO #OVERLAP_MEBER_LIST1 
	FROM (  
			SELECT [T].[USER_ID]
			FROM [TB_ORDER] AS [T] 
			...
			WHERE (
					[T].[PAYMENT_METHOD_CODE] IS NOT NULL 
					AND (([T].[PAYMENT_METHOD_CODE] <> '') OR [T].[PAYMENT_METHOD_CODE] IS NULL)
	...
	FROM (  
			...
			FROM [TB_ORDER] AS [T]
					...
					INNER JOIN [TB_INVITATION_DETAIL] AS [T7] ON [T6].[INVITATION_ID] = [T7].[INVITATION_ID]
					LEFT JOIN [TB_REFUND_INFO] AS [T8] ON [T].[ORDER_ID] = [T8].[ORDER_ID]
			WHERE ([T].[PAYMENT_METHOD_CODE] IS NOT NULL AND 
						(
 ...
 -- 중복구매한 회원아이디 
	SELECT * INTO #OVERLAP_MEBER_LIST2 
	FROM (  
			 SELECT [T].[USER_ID]
			 FROM TB_ORDER AS T
					INNER JOIN TB_ORDER_PRODUCT AS T0 ON T.ORDER_ID = [T0].[ORDER_ID]
			...
			WHERE	([T].[PAYMENT_METHOD_CODE] IS NOT NULL AND (([T].[PAYMENT_METHOD_CODE] <> '') OR [T].[PAYMENT_METHOD_CODE] IS NULL)) 
				...
				IF @SEARCHPAYMENTSTATUS = 'PSC03_PSC05' BEGIN -- 결제상태검색 (취소/환불) 
							...
							IF @SEARCHTXT IS NOT NULL AND @SEARCHTXT <> '' BEGIN  --검색어검색 
									...
									IF @SEARCHPERIOD = 'ORDER' BEGIN  -- 회원/ 분류/ 브랜드/ (취소/환불) /검색어
												...
												SELECT * 
												FROM #ORDER_LIST1 A INNER JOIN #OVERLAP_MEBER_LIST1 B ON A.USER_ID = B.USER_ID
												WHERE A.[USER_ID] <> ''  AND (A.PRODUCT_CATEGORY_CODE LIKE '' OR (CHARINDEX(A.PRODUCT_CATEGORY_CODE, @SEARCHKIND) > 0)) AND 
														(A.PRODUCT_BRAND_CODE LIKE '' OR (CHARINDEX(A.PRODUCT_BRAND_CODE, @SEARCHBRAND) > 0)) AND  
														(A.PAYMENT_STATUS_CODE LIKE '' OR (CHARINDEX(A.PAYMENT_STATUS_CODE, @SEARCHPAYMENTSTATUS) > 0)) AND 
														(
														(
															(
														...
														) OR (@SEARCHTXT LIKE '' OR CHARINDEX(@SEARCHTXT, A.[USER_ID]) > 0)
														)
												ORDER BY  A.[USER_ID] ASC, REGIST_DATETIME1 DESC

									END ELSE BEGIN   -- 결제일로 검색
											
												SELECT * 
														FROM #ORDER_LIST2  A INNER JOIN #OVERLAP_MEBER_LIST2 B ON A.USER_ID = B.USER_ID
														WHERE A.[USER_ID] <> ''  AND (A.PRODUCT_CATEGORY_CODE LIKE '' OR (CHARINDEX(A.PRODUCT_CATEGORY_CODE, @SEARCHKIND) > 0)) AND 
															  (A.PRODUCT_BRAND_CODE LIKE '' OR (CHARINDEX(A.PRODUCT_BRAND_CODE, @SEARCHBRAND) > 0)) AND  
															  (A.PAYMENT_STATUS_CODE LIKE '' OR (CHARINDEX(A.PAYMENT_STATUS_CODE, @SEARCHPAYMENTSTATUS) > 0)) AND 
															  (
												...
												ORDER BY A.[USER_ID] ASC,	PAYMENT_DATETIME DESC
									END 
							...
							END ELSE BEGIN  --검색어X 
									...
									IF @SEARCHPERIOD = 'ORDER' BEGIN    -- 회원/ 분류/ 브랜드/ (취소/환불)
												SELECT * 
														...
														WHERE A.[USER_ID] <> ''  AND (A.PRODUCT_CATEGORY_CODE LIKE '' OR (CHARINDEX(A.PRODUCT_CATEGORY_CODE, @SEARCHKIND) > 0)) AND 
															  (A.PRODUCT_BRAND_CODE LIKE '' OR (CHARINDEX(A.PRODUCT_BRAND_CODE, @SEARCHBRAND) > 0)) AND  
															  (A.PAYMENT_STATUS_CODE LIKE '' OR (CHARINDEX(A.PAYMENT_STATUS_CODE, @SEARCHPAYMENTSTATUS) > 0)) 
												ORDER BY  A.[USER_ID] ASC,REGIST_DATETIME1 DESC
				...
				END ELSE IF @SEARCHPAYMENTSTATUS != 'ALL' BEGIN   -- 결제상태검색 (결제완료/입금대기)
							...
							IF @SEARCHTXT IS NOT NULL AND @SEARCHTXT <> '' BEGIN  --검색어 
									...
									IF @SEARCHPERIOD = 'ORDER' BEGIN   -- 주문일로 검색 -- 회원/ 분류/ 브랜드/ (결제완료/입금대기) /검색어
										SELECT * 
										FROM #ORDER_LIST1   A INNER JOIN #OVERLAP_MEBER_LIST1 B ON A.USER_ID = B.USER_ID
										WHERE A.[USER_ID] <> ''  AND (PRODUCT_CATEGORY_CODE LIKE '' OR (CHARINDEX(PRODUCT_CATEGORY_CODE, @SEARCHKIND) > 0)) AND 
