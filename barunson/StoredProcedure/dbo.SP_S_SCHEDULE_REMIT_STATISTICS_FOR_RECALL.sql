IF OBJECT_ID (N'dbo.SP_S_SCHEDULE_REMIT_STATISTICS_FOR_RECALL', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_S_SCHEDULE_REMIT_STATISTICS_FOR_RECALL
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[SP_S_SCHEDULE_REMIT_STATISTICS_FOR_RECALL]
	@yynm VARCHAR(8) = NULL
AS
BEGIN
	...
	SET @Date = ISNULL(@yynm, CONVERT(VARCHAR(10), DATEADD(MONTH, -1, GETDATE()), 112))
	SET @Month = LEFT(@Date, 6)
	SET @Year = LEFT(@Date, 4)

	/* 일 데이터 삭제 */
	DELETE FROM TB_Remit_Statistics_Daily WHERE Date LIKE @Month+'%'
	...
	FROM TB_Standard_Date AS SD
		...
		LEFT JOIN (
			...
			where depositorHits.Status_Code is not null
				and depositorHits.Request_Date like @Month +'%'
		...
		LEFT JOIN (
			...
			from TB_Account
			where left(convert(varchar, Regist_DateTime, 112), 6) = @Month
		...
		LEFT JOIN (
			...
			where remit.Result_Code = 'RC005'
				and remit.Complete_Date like @Month+'%'
