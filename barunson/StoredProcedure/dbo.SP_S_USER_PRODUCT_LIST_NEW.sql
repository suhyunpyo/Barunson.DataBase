IF OBJECT_ID (N'dbo.SP_S_USER_PRODUCT_LIST_NEW', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_S_USER_PRODUCT_LIST_NEW
GO
...
GO
CREATE PROCEDURE [dbo].[SP_S_USER_PRODUCT_LIST_NEW]
/***************************************************************
작성자	:	표수현
작성일	:	2020-02-15
...
AS
 ...
 IF @SELECTED_CATEGORY_COUNT > 0 BEGIN
	...
	SELECT	UPDATE_DATETIME = B.UPDATE_DATETIME, 
			...
			TOTALSALE = CASE WHEN @SORT_GUBUN = 2 Then (SELECT COUNT(*) FROM TB_ORDER_PRODUCT Q WHERE Q.PRODUCT_ID = A.PRODUCT_ID) Else 0 End,
			SORT = A.SORT
 ...
 END ELSE BEGIN
	...
	FROM	TB_PRODUCT_CATEGORY A INNER JOIN 
			TB_PRODUCT B ON A.PRODUCT_ID = B.PRODUCT_ID INNER JOIN 
			TB_COMMON_CODE C ON B.PRODUCT_BRAND_CODE = C.CODE
	WHERE  B.DISPLAY_YN = 'Y' AND A.CATEGORY_ID = @CATEGORY_ID
	AND		C.CODE= (CASE WHEN @CONDITION1 = 1 THEN @SEARCHBRAND_CODE ELSE C.CODE END)
 ...
 ORDER BY   
			...
			CASE WHEN @SORT_GUBUN = 2 THEN TB.TOTALSALE  END DESC  ,
