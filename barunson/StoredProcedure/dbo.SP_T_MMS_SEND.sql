IF OBJECT_ID (N'dbo.SP_T_MMS_SEND', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_T_MMS_SEND
GO
...
GO
CREATE PROCEDURE [dbo].[SP_T_MMS_SEND]
/***************************************************************
작성자	:	표수현
작성일	:	2020-02-15
...
초대장 ->  "SELECT GROUPNAME FROM MCARD_INVITATIONPARTY WHERE INVITATIONID = " & INVITATIONID
 ...
 IF @MMS_SEND_YN = 'N' BEGIN 
	...
	SELECT @PRODUCT_TYPE_CODE= F.CODE, 
		@GROOMNAME = GROOM_NAME, 
		@BRIDENAME=  BRIDE_NAME,
		@INVITATION_URL = 'https://www.barunsonmcard.com/m/' + C.INVITATION_URL,
		@MAIN_IMG_URL = 'http://www.barunsonmcard.com' + E.MAIN_IMAGE_URL
	FROM TB_ORDER A INNER JOIN 
		 TB_INVITATION B ON A.ORDER_ID = B.ORDER_ID INNER JOIN 
		 TB_INVITATION_DETAIL C ON B.INVITATION_ID = C.INVITATION_ID INNER JOIN 
		 TB_ORDER_PRODUCT D ON A.ORDER_ID = D.ORDER_ID  INNER JOIN 
	...
	SELECT @MSG = C.Invitation_Title + CHAR(13) + CHAR(10) + @INVITATION_URL,
		   @MAIN_IMG_URL = 'http://www.barunsonmcard.com' + C.SNS_IMAGE_URL
	FROM	TB_ORDER A INNER JOIN  
			TB_INVITATION B ON A.ORDER_ID = B.ORDER_ID INNER JOIN  
			TB_INVITATION_DETAIL C ON B.INVITATION_ID= C.INVITATION_ID
	WHERE A.ORDER_ID = @ORDER_ID
...
--SELECT MSG  =   @MSG
 ...
 --SET @MAIN_IMG_URL = 'https://resources.barunsonmcard.com/upload/invitation/210825/111/e77473a8-c7a1-4c2d-bf34-f220f3c25b7b.jpg'
--SET @MAIN_IMG_URL = 'https://resources.barunsonmcard.com/upload/invitation/210825/111/6037ff5d-6e10-4908-bb43-389b476ae23b.png'
...
--SELECT MAIN_IMG_URL   =  @MAIN_IMG_URL
	...
	EXEC BAR_SHOP1.DBO.SP_EXEC_SMS_OR_MMS_SEND @SEND_PNUM, @RECV_PNUM, @TITLE, @MSG, '', '고객 사용', '모바일초대장 핸드폰으로 주소 보내기 - SP_DACOMMMS', '', 1, @MAIN_IMG_URL
