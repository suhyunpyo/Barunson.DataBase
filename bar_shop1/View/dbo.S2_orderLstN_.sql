IF OBJECT_ID (N'dbo.S2_orderLstN_', N'V') IS NOT NULL DROP View dbo.S2_orderLstN_
GO
...
AS
	SELECT
		...
		--, ISNULL(CCG.cnt,0) ccg_cnt
		, CCG.member_id_
		--,(SELECT count(*) cnt FROM CUSTOM_ETC_ORDER WHERE A.member_id = ISNULL(member_id,'') and order_type = '3') cnt
	FROM   custom_order AS A
		...
		LEFT JOIN
		(
			SELECT member_id member_id_ FROM CUSTOM_ETC_ORDER WHERE ISNULL(member_id,'') <> '' and order_type = '3' GROUP BY member_id
		) AS CCG ON A.member_id = CCG.member_id_
