IF OBJECT_ID (N'dbo.SP_EVENT_BARUNSON_NEWCARD_VOTE_MAX', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_EVENT_BARUNSON_NEWCARD_VOTE_MAX
GO
...
CREATE PROCEDURE [dbo].[SP_EVENT_BARUNSON_NEWCARD_VOTE_MAX]  
 @UID        AS VARCHAR(50),  
 ...
 BEGIN  
  ...
  IF @DAY_COUNT = (@DAY_MAX-1) AND NOT EXISTS(  
    ...
    ELSE  
        BEGIN  
        SET @RESULT_CODE = '0000'  
        SET @RESULT_MESSAGE = '5종의 제품에 투표 완료하면 쿠폰이 발급됩니다.'  
        END  
    END  
ELSE  
    BEGIN  
