IF OBJECT_ID (N'dbo.SP_SELECT_S2_CARD_WITH_ERP_STOCK', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_SELECT_S2_CARD_WITH_ERP_STOCK
GO
...
exec sp_executesql N' EXEC SP_SELECT_S2_CARD_WITH_ERP_STOCK    @P_CARD_SET_PRICE_MIN  ,  @P_CARD_SET_PRICE_MAX  ,  @P_CARD_KIND_SEQ  ,  @P_CARD_BRAND  ,  @P_CARD_CODE_OR_CARD_NAME  ,  @P_PRODUCTION_STATUS_NAME  ,  @P_SAMPLE_ORDER_AVAILABLE_YORN  ,  @P_SIT

E_SEARCH_VALUE  ,  @P_SALES_TYPE  ,  @P_MIN_STOCK_UNDER_YORN  ,  @P_PAGE_SIZE  ,  @P_PAGE_NUMBER  ,  @P_ORDER_BY_NAME  ,  @P_ORDER_BY_TYPE ',N'@P_CARD_SET_PRICE_MIN int,@P_CARD_SET_PRICE_MAX int,@P_CARD_KIND_SEQ int,@P_CARD_BRAND nvarchar(4000),@P_CARD_CO

DE_OR_CARD_NAME nvarchar(4000),@P_PRODUCTION_STATUS_NAME nvarchar(25),@P_SAMPLE_ORDER_AVAILABLE_YORN nvarchar(4000),@P_SITE_SEARCH_VALUE nvarchar(29),@P_SALES_TYPE nvarchar(4000),@P_MIN_STOCK_UNDER_YORN nvarchar(1),@P_PAGE_SIZE int,@P_PAGE_NUMBER int,@P_O

RDER_BY_NAME nvarchar(8),@P_ORDER_BY_TYPE nvarchar(4)',@P_CARD_SET_PRICE_MIN=0,@P_CARD_SET_PRICE_MAX=99999,@P_CARD_KIND_SEQ=0,@P_CARD_BRAND=N'',@P_CARD_CODE_OR_CARD_NAME=N'',@P_PRODUCTION_STATUS_NAME=N'정상판매|단종예정|발주대기|폐기대상|폐기|공백',@P_SAMPLE_ORDER_AVAILABLE_

YORN=N'',@P_SITE_SEARCH_VALUE=N'5001|5006|5007|5003|5000|5008',@P_SALES_TYPE=N'',@P_MIN_STOCK_UNDER_YORN=N'N',@P_PAGE_SIZE=99999,@P_PAGE_NUMBER=1,@P_ORDER_BY_NAME=N'REG_DATE',@P_ORDER_BY_TYPE=N'DESC'

*/
CREATE PROCEDURE [dbo].[SP_SELECT_S2_CARD_WITH_ERP_STOCK]
...
BEGIN
	...
	SELECT	DISTINCT 
        ...
        ,   (SELECT CODE_VALUE FROM manage_code WHERE CODE_TYPE ='print_mount' AND  CODE =  SUBSTRING(ISNULL(SCO.PrintMethod,''),1,1)) AS PrintMethod1 --박 
...
OR  SCES.PRODUCTION_STATUS_NAME IN (SELECT PRODUCTION_STATUS_NAME FROM @T_PRODUCTION_STATUS_NAME)
	...
	SELECT @TOTAL_COUNT = COUNT(1) FROM #TEMP_TABLE
