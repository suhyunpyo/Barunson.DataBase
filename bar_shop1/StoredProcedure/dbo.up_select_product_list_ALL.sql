IF OBJECT_ID (N'dbo.up_select_product_list_ALL', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_product_list_ALL
GO
...
*/
-- =============================================
CREATE Procedure [dbo].[up_select_product_list_ALL]
...
BEGIN
	...
	select top (@pagesize) A.card_seq, B.card_code, B.card_name, B.CardBrand, B.cardset_price, B.card_price, B.regdate,	--0~5
		...
		(select count(seq) from S2_usercomment with(nolock) where company_seq=@company_seq and card_seq=A.card_seq) AS comment_cnt,
		isnull((select avg(score) from S2_usercomment with(nolock) where company_seq=@company_seq and card_seq=A.card_seq group by card_seq),0) AS comment_avg,
		(select count(D.SAMPLE_ORDER_SEQ) from CUSTOM_SAMPLE_ORDER AS C with(nolock) join  CUSTOM_SAMPLE_ORDER_ITEM AS D with(nolock) on C.sample_order_seq=D.SAMPLE_ORDER_SEQ where C.COMPANY_SEQ=@company_seq and card_seq=A.card_seq  ) AS sample_cnt
