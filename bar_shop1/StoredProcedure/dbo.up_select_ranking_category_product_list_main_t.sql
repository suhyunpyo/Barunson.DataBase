IF OBJECT_ID (N'dbo.up_select_ranking_category_product_list_main_t', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_ranking_category_product_list_main_t
GO
...
--  
-- up_select_ranking_category_product_list_main 5007,  300    
-- =============================================    
  
CREATE PROCEDURE [dbo].[up_select_ranking_category_product_list_main_t]  
...
-- WHERE b.st_company_seq=5007 AND a.rk_st_seq=8   
 ...
 SELECT top 14  
  ...
  , REPLACE(CONVERT(VARCHAR,CONVERT(MONEY,(round((B.CardSet_Price * ((100 - D.Discount_Rate) * 0.01)) , 0) * 1)),1), '.00', '') Discount_Card_Price_v2  
 FROM S4_Ranking_Sort_Table a   
  LEFT JOIN S4_Ranking_Sort r ON a.rk_st_seq=r.st_code   
  LEFT JOIN S2_Card b ON a.rk_card_code=b.card_seq   
  LEFT outer join S2_CardSalesSite AS c with(nolock) on A.rk_card_code=c.card_seq and c.company_seq=@company_seq   
