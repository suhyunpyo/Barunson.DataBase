...
/*
	...
	EXEC SP_COUPON_COMPLETE 's4guest', 2472827, '234455|234457', 116510
*/

ALTER PROCEDURE [dbo].[SP_COUPON_COMPLETE]
		@P_UID									AS VARCHAR(50)
	,	@P_ORDER_SEQ							AS INT
...
BEGIN
	...
	BEGIN
		...
		IF @P_TOTAL_PRICE = @P_SETTLE_PRICE
		BEGIN
			-- 쿠폰보관함에 쿠폰을 사용완료로 변경 (N)
			UPDATE	COUPON_ISSUE 
			SET		ACTIVE_YN = 'N'
			FROM	COUPON_ISSUE CI
			JOIN	#TempTable T	ON CI.COUPON_ISSUE_SEQ  = T.COUPON_ISSUE_SEQ
			WHERE	CI.UID = @P_UID
			AND		CI.COMPANY_SEQ = @COMPANY_SEQ
			
			-- 주문 시 사용 쿠폰 CUSTOM_ORDER_COUPON에 저장
			INSERT INTO CUSTOM_ORDER_COUPON (COUPON_ISSUE_SEQ, ORDER_SEQ, COUPON_AMT, REG_DATE)
			SELECT COUPON_ISSUE_SEQ, ORDER_SEQ, DISCOUNT_TOTAL_PRICE, GETDATE() AS REG_DATE from #TempTable; 

			-- 쿠폰을 사용한 총 결제금액 UPDATE

			DECLARE @P_REDUCE_PRICE	INT
			SET @P_REDUCE_PRICE = (SELECT SUM(DISCOUNT_TOTAL_PRICE) DISCOUNT_TOTAL_PRICE FROM #TempTable)

			UPDATE CUSTOM_ORDER
			SET	LAST_TOTAL_PRICE = @P_SETTLE_PRICE, REDUCE_PRICE = '-'+CONVERT(CHAR,@P_REDUCE_PRICE)
			WHERE ORDER_SEQ = @P_ORDER_SEQ 

			SET @RESULT_CODE	= '0000'
			SET @RESULT_MESSAGE = '쿠폰이 사용완료되었습니다.'		
		END
		ELSE
		BEGIN
			SET @RESULT_CODE	= '9999'
			SET @RESULT_MESSAGE = '금액이 일치하지 않습니다.'					
		END
