IF OBJECT_ID (N'dbo.sp_stat_CardSumBrandPrice', N'P') IS NOT NULL DROP PROCEDURE dbo.sp_stat_CardSumBrandPrice
GO
...
CREATE   Procedure [dbo].[sp_stat_CardSumBrandPrice]
	@sales_gubun char(1),	
	@sdate 	char(8),		-- 검색 시작일
	@edate  char(8)		-- 검색 종료일
as
...
if @sales_gubun = ''
	begin
		select  b.company ,b.card_price,IsNull(sum(order_count),0) as sum_count, IsNull(sum(settle_price),0) as sum_price  
		from 
		(
			select order_seq,card_seq,order_count,settle_price from custom_order 
			where Convert(char(8),src_send_date,112) between @sdate and @edate and sales_gubun in ('W','T','U','A','B','S')
		) a Right Join (select distinct company,card_seq,
										card_price = Case
						...
						from card where card_cate='I1' and card_price_customer>0) b
		on a.card_seq = b.card_seq
		group by b.company,b.card_price	
		order by b.company,b.card_price	
	end
else
	
	begin			
		select  b.company ,b.card_price,IsNull(sum(order_count),0) as sum_count, IsNull(sum(settle_price),0) as sum_price  from 
		(
		select order_seq,card_seq,order_count,settle_price from custom_order 
