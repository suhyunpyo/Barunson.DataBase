...
ALTER PROCEDURE [dbo].[PROC_SELECT_MARKETING_TARGET_MEMBER_VER2]
...
--SET @P_REG_E_DT = ISNULL(@P_REG_E_DT, convert(varchar, GETDATE(), 23))
--SET @P_WED_E_DT = ISNULL(@P_WED_E_DT, convert(varchar, GETDATE(), 23))
...
SELECT
	...
	, CASE SAMPLE_YN WHEN 'Y' THEN (SELECT replace(replace(SUBSTRING(T.COL1,2, LEN(T.COL1)-1), '[', ''), ']','') SAMPLE_SITE
		FROM (
		SELECT (SELECT ',' + QUOTENAME(ISNULL(SALES_GUBUN,'SB')) FROM CUSTOM_SAMPLE_ORDER WITH(NOLOCK) where member_id = T2.USER_ID group by SALES_GUBUN FOR XML PATH('')) COL1
		) T	
		...
		SELECT 
			(
					SELECT CASE WHEN ISNULL(SELECT_SALES_GUBUN, '') = '' THEN     
						   ...
						   ELSE      
							...
							END    
					END
					FROM S2_USERINFO_THECARD WITH(NOLOCK) WHERE uid = T1.uid
			) AS SITE_DIV
			 , CONVERT(VARCHAR(20), Reg_Date, 20) AS REGDATE
			 , UNAME AS NAME
			 ...
			 , address_d AS ADDR
			 , WEDDING_DAY 
			 , ISNULL((SELECT TOP  1 'Y' FROM CUSTOM_SAMPLE_ORDER WITH(NOLOCK) WHERE member_id = uid AND status_Seq > 0  AND status_seq NOT IN ('3','5') ),'N') AS SAMPLE_YN
			 , ISNULL((SELECT case when count(1) > 0 then 'Y' else 'N' end FROM CUSTOM_ORDER WITH(NOLOCK) WHERE member_id = uid AND status_Seq > 0 AND status_seq NOT IN ('3','5') AND order_type IN ('1','6','7') ),'N') AS ORDER_YN
			 , ISNULL((SELECT case when count(1) > 0 then 'Y' else 'N' end FROM CUSTOM_ORDER WITH(NOLOCK) WHERE member_id = uid AND status_Seq > 0 AND status_seq NOT IN ('3','5') AND order_type= '2' ),'N') AS ORDER_T_YN
			 , chk_sms AS SMS_YN
			 , ISNULL(MarketingYN, 'N') AS MARKETING_YN
			 , ISNULL((SELECT case when count(1) > 0 then 'Y' else 'N' end FROM S2_USERINFO_THIRD_PARTY_MARKETING_AGREEMENT as m119001 WHERE m119001.[uid] = T1.[uid] and MARKETING_TYPE_CODE = '119001' and USE_YORN = 'Y' ),'N') AS M119001_YN
			 , ISNULL((SELECT case when count(1) > 0 then 'Y' else 'N' end FROM S2_USERINFO_THIRD_PARTY_MARKETING_AGREEMENT as m119006 WHERE m119006.[uid] = T1.[uid] and MARKETING_TYPE_CODE = '119006' and USE_YORN = 'Y' ),'N') AS M119006_YN
  			 , SAMSUNG_MEM_YN
			 , LG_MEM_YN
			 , CASAMIA_MEM_YN
			 , CUCKOO_MEM_YN
		 ...
		 FROM ( 
				SELECT CASE WHEN INTEGRATION_MEMBER_YORN = 'N' THEN row_number() over(partition by A.HPHONE order by A.reg_date)
					 ...
					 , WEDD_NAME
					 , UMAIL
					 , CASE WHEN C.UserId IS NOT NULL THEN 'Y' ELSE 'N' END AS Roulette_YN
					 , CASE WHEN Chungho.UserId IS NOT NULL THEN 'Y' ELSE 'N' END AS Chungho_YN
				  FROM ( 
						SELECT SITE_DIV_NAME
						 ...
						 WHERE LEN(A.HPHONE) > 12
						  ...
						  AND (@P_REG_E_DT IS NULL OR A.INTERGRATION_DATE < convert(varchar,DATEADD(DAY,1,CONVERT(DATE, @P_REG_E_DT))))  
						  AND (@P_SMS_YN IS NULL OR A.chk_sms = @P_SMS_YN)
						  AND (@P_CHK_MAILSERVICE IS NULL OR A.chk_mailservice = @P_CHK_MAILSERVICE)
						) AS A 
						LEFT OUTER JOIN S4_Event_Raina   AS B WITH(NOLOCK) ON (A.UID = B.UID AND B.event_div = 'MKevent')
						LEFT OUTER JOIN S2_UserInfo_Jehu AS C WITH(NOLOCK) ON (A.UID = C.UserId AND C.PartnerCode = 'enmad')  -- 룰렛
						LEFT OUTER JOIN S2_UserInfo_Jehu AS Chungho WITH(NOLOCK) ON (A.UID = Chungho.UserId AND Chungho.PartnerCode = 'ChhoEvent')  --청호나이스
					) AS T1 
				WHERE RM = 1 
			) T2
...
WHERE 1 = 1
  ...
  AND (@P_WED_E_DT IS NULL OR WEDDING_DAY < convert(varchar, DATEADD(DAY,1,CONVERT(DATE, @P_WED_E_DT))))
