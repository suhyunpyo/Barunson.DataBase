IF OBJECT_ID (N'dbo.SP_SELECT_OUTSOURCING_ORDER_MST_FOR_LIST_PREMIER_TEST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_SELECT_OUTSOURCING_ORDER_MST_FOR_LIST_PREMIER_TEST
GO
...
EXEC SP_SELECT_OUTSOURCING_ORDER_MST_FOR_LIST_PREMIER_TEST '', '', '', '', '', '', '', '2016-12-20', '2017-11-27','','','14336'
13350,13349,13348
*/

CREATE PROCEDURE [dbo].[SP_SELECT_OUTSOURCING_ORDER_MST_FOR_LIST_PREMIER_TEST]
	...
	IF @P_EXCEL_ORDER_MST_SEQS <> '' 
		BEGIN
			...
			SELECT  ROW_NUMBER() OVER(ORDER BY A.REG_DATE ASC) AS ROW_NUM
				...
				, (SELECT STUFF((SELECT '|' + D.ZIP +'^' + D.ADDR+'^' + D.ADDR_DETAIL+'^' + D.DELIVERY_COM+'^' + D.DELIVERY_CODE_NUM+'^' + CONVERT(VARCHAR(10),D.DELIVERY_DATE, 121)+'^' + CONVERT(VARCHAR, D.DELIVERY_SEQ)
				...
				, B.settle_method         as SETTLE_METHOD --결제방법
				, B.settle_price          as SETTLE_PRICE --결제금액
				, B.settle_status         as SETTLE_STATUS --결제상태
				, B.order_type            as ORDER_TYPE --청첩장구분 (청첩장/감사장)
				...
				, (SELECT STUFF((SELECT '|' + COI.ITEM_TYPE +'^'+ S2C.CARD_NAME +'^'+ S2C.Card_Div +'^'+ CONVERT(VARCHAR, COI.item_count)+'^' + CONVERT(VARCHAR,(COI.item_sale_price * COI.item_count ))
					  FROM custom_order_item COI
					   LEFT JOIN S2_CARD S2C ON COI.card_seq = S2C.Card_Seq
