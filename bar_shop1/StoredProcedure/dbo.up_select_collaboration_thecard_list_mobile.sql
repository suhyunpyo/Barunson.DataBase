IF OBJECT_ID (N'dbo.up_select_collaboration_thecard_list_mobile', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_collaboration_thecard_list_mobile
GO
...
-- Description :     
-- up_select_collaboration_thecard_list_mobile 5007, null, 99, 1, 40, '', 'ASC', 100      
-- =============================================      
CREATE PROCEDURE [dbo].[up_select_collaboration_thecard_list_mobile]    
...
BEGIN    
 ...
 BEGIN    
 DECLARE @CATEGORY_PARAM AS VARCHAR(20)      
 DECLARE @TEMP TABLE     
 ( CategoryParam INT)    
 ...
 IF @category = 99    
  BEGIN    
   ...
   --SELECT 85 union all 레핀주석
   SELECT 72 union all    
   SELECT 84    
  END     
 ELSE      
  BEGIN    
   INSERT INTO @temp values(@category)    
  END     
  ...
  WHERE 1 = 1    
    AND A.RK_ST_SEQ in ( SELECT CategoryParam FROM @TEMP )    
