IF OBJECT_ID (N'dbo.SP_SELECT_CUSTOM_ORDER_PLIST_FOR_BACK_COREL_20210827', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_SELECT_CUSTOM_ORDER_PLIST_FOR_BACK_COREL_20210827
GO
...
CREATE PROCEDURE [dbo].[SP_SELECT_CUSTOM_ORDER_PLIST_FOR_BACK_COREL_20210827]  
    @P_ORDER_SEQ AS INT  
...
BEGIN  
    ...
    SELECT  A.ID  
        ...
        ,   CASE WHEN A.PRINT_TYPE IN ('C', 'P','I') AND B.ERP_CODE IN (SELECT ERP_CODE FROM S2_CardCorelTemplateInfo)  
                 THEN B.OLD_CODE  
            ELSE B.ERP_CODE  
            END AS CARD_CODE  
