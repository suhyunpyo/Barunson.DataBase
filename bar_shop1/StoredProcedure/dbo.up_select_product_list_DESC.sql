IF OBJECT_ID (N'dbo.up_select_product_list_DESC', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_product_list_DESC
GO
...
*/
-- =============================================
CREATE Procedure [dbo].[up_select_product_list_DESC]
...
BEGIN
	...
	select top (@pagesize) A.card_seq, B.card_code, B.card_name, C.isNew, B.cardset_price, B.card_price,
		...
		CASE @orderby 
				...
				WHEN 'SORT' THEN A.sorting_num
				WHEN 'PRICE' THEN B.CardSet_Price END
		)
		 DESC
		) 
		-- ============= not in end ===============
