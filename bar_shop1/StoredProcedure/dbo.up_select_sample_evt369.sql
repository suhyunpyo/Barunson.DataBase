IF OBJECT_ID (N'dbo.up_select_sample_evt369', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_sample_evt369
GO
...
-- Description:	시즌2 관리자, 샘플이용후기 369이벤트
-- EXEC up_select_sample_evt369 @Sales_Gubun='SA,B,C', @sdate='2015-12-15', @edate='2016-01-16',  @search_string='', @chk='0^1^2', @page=1, @pagesize=20, @ER_Type=0, @allchk_comp='1' 
-- =============================================
CREATE PROCEDURE [dbo].[up_select_sample_evt369]
...
BEGIN
	...
	BEGIN
		...
		SELECT COUNT(ER_Idx) 
		FROM S4_Event_Review AS A WITH(NOLOCK) INNER JOIN Evt_three_six_nine_board AS C WITH(NOLOCK)
		ON A.ER_Userid = C.B_Uid
		...
		SELECT TOP (@pagesize)
			...
			, B.SALES_GUBUN
			, A.inflow_route
			,(SELECT COUNT(*) FROM S4_Event_Review AS AA INNER JOIN S4_Event_Review_Status BB ON AA.ER_idx = BB.ERA_ER_idx WHERE AA.ER_USERID = A.ER_USERID and BB.ERA_Status = '1' ) cc
		FROM S4_Event_Review AS A WITH(NOLOCK)
			INNER JOIN Company AS B WITH(NOLOCK)
				ON A.ER_Company_Seq = B.COMPANY_SEQ
			 INNER JOIN Evt_three_six_nine_board AS C WITH(NOLOCK)
				ON A.ER_Userid = C.B_Uid
		WHERE ER_Idx NOT IN 
		(
