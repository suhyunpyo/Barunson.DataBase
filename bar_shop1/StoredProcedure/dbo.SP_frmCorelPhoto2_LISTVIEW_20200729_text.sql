IF OBJECT_ID (N'dbo.SP_frmCorelPhoto2_LISTVIEW_20200729_text', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_frmCorelPhoto2_LISTVIEW_20200729_text
GO
...
CREATE PROCEDURE [dbo].[SP_frmCorelPhoto2_LISTVIEW_20200729_text]
...
--EXEC SP_frmCorelPhoto2_LISTVIEW '', '', '3', '1', '0', '', '', NULL, NULL, '2', '20190720', '20190730'
-- EXEC SP_frmCorelPhoto2_LISTVIEW '', '', '8', '1', '0', '2019-07-29', '2019-07-30', 0, 0, '3', '20190730', '20190730', '0'

--	EXEC SP_frmCorelPhoto2_LISTVIEW '', '', '0', '0', '1', '2019-07-31', '2019-07-31', 0, 0, '3', '20190730', '20190730', '2'
...
--WHERE A.order_seq IN ( SELECT DISTINCT A.order_seq FROM #TAESAN_SUB_CARD A JOIN custom_order_plist B ON A.order_seq = B.order_seq AND B.isNotPrint = 0 )

--AND CD.Card_Code IN ( 'BH9703A', 'BH9704A' )


--select @bWhereBypass, @ORDER_TYPE, @SrchCase, @SrchSDate, @SrchEDate
--	, @ChasuSDATE, @ChasuEDATE , @ChasuS, @ChasuE
--	, @ChasuChk, @PrintCompleted

--select * from #TAESAN_SUB_CARD
...
*/
		...
		*/
		...
		AND ( 
			...
			OR	( @ORDER_TYPE = '9' AND A.order_type = '7' AND B.card_code IN (SELECT HardCode FROM HardCodingList WHERE HardID = 'SAMSUNG_GOLDFOIL' AND HardUse = 'Y') )
		)
		/*
		--원모어띵 검색시
		AND ( 
		...
		AND ( 
			...
			OR	( @ORDER_TYPE = '12' AND C.isMasterDigital = '1' )
		)
		...
		AND ( 
			...
			OR	( @ORDER_TYPE = '14' AND A.order_type in ('6','7') AND C.isInternalDigital = '1'  )		-- AND A.order_type = '6' 일단 주석처리.
		)
...
--ORDER BY ( CASE WHEN @OrderBy = '0' THEN A.src_confirm_Date ELSE 1 END )
--	, ( CASE WHEN @OrderBy = '1' THEN E.pseq ELSE 1 END )
--	, ( CASE WHEN @OrderBy = '2' THEN A.order_seq  ELSE 1 END )
--	, ( CASE WHEN @OrderBy = '3' THEN B.card_code  ELSE '1' END )
--	, ( CASE WHEN @OrderBy NOT IN ( '0', '1', '2', '3') THEN B.card_code  ELSE '1' END )





-- EXEC SP_frmCorelPhoto2_LISTVIEW '', '', '0', '0', '0', '2019-07-30', '2019-07-30', 0, 0, '3', '20190730', '20190730', '0'

--select * from custom_order_plist where order_seq in ( 2821202, 2826387 )

--select * from custom_order_plist where order_seq =  2825585 
--

--select * from custom_order_plist where order_seq =  2825585 
--select * from custom_order_plist where order_seq =  2820922 
--select * from custom_order_plist where order_seq =  2821512 

--select * from custom_order_plist WHERE isNotPrint = 0

--select * from custom_order where order_seq =  2825585 

	--    EXEC SP_frmCorelPhoto2_LISTVIEW '', '', '0', '1', '0', '2019-07-30', '2019-07-30', 0, 0, '2', '20190730', '20190730', '0'
	--		EXEC SP_frmCorelPhoto2_LISTVIEW '', '', '0', '0', '0', '2019-07-30', '2019-07-30', 0, 0, '2', '20190722', '20190730', '2'


	--select A.order_type, A.order_seq, A.status_seq
	--	   , E.pseq
	--	   , E.pdate
	--	    , A.order_date
	--	 , A.src_closecopy_date
	--	 , B.card_code
	--, ISNULL(C.isInternalDigital, '')
	--, C.isCustomDColor
	--, D.WEPOD_YORN
	--FROM Custom_Order AS A 
	--JOIN S2_Card AS B ON A.Card_Seq = B.Card_Seq
