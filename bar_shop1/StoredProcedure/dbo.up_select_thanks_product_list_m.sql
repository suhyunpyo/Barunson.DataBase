IF OBJECT_ID (N'dbo.up_select_thanks_product_list_m', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_thanks_product_list_m
GO
...
-- =============================================  
Create PROCEDURE [dbo].[up_select_thanks_product_list_m]  
 ...
 */  
 ...
 (  
  SELECT  ROW_NUMBER() OVER (ORDER BY RegDate DESC ) AS RowNum  
    ,A.Card_Name  
    ,A.Card_Code  
    ,(select code_value from manage_code where code_type='cardbrand' and code = A.CardBrand ) as CardBrand  
    ,A.CardSet_Price  
    ,A.card_seq  
    ,A.RegDate      
