IF OBJECT_ID (N'dbo.SP_INSERT_FREE_GIFT_200825', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_INSERT_FREE_GIFT_200825
GO
...
SP_INSERT_FREE_GIFT_200825 3026020 
*/    
    
CREATE PROCEDURE [dbo].[SP_INSERT_FREE_GIFT_200825]    
    @ORDER_SEQ                  AS INT    
...
BEGIN    
    ...
    BEGIN    
	-- // 더카드 트래블패키지 (103(베이지).104(민트))  
	IF @FREE_GIFT_SEQ = '103' or @FREE_GIFT_SEQ = '104'  
		BEGIN  
			IF EXISTS (  
						...
						AND card_seq in (select card_seq from S4_MD_Choice where md_seq = 777 )   
				)  
				BEGIN                     
					SET @FLOW = 0  
				END  
			ELSE  
				...
				END    
		END   
  select @FLOW as flow5

	--// (THECARD) cup_coaster_GIFT 105  
	IF @FREE_GIFT_SEQ = '105'  
		BEGIN  
			IF EXISTS (  
						SELECT TOP 1 A.ORDER_SEQ  
						FROM CUSTOM_ORDER A   
			...
			ELSE  
				...
				END    
		END   
select @FLOW as flow4
    -- 프리지아 향수   
    IF @FREE_GIFT_SEQ = '112'   
		BEGIN  
			IF EXISTS (  
					SELECT TOP 1 A.ORDER_SEQ  
					FROM CUSTOM_ORDER A   
...
select @FLOW as flow3
	...
	select @FREE_GIFT_TARGET_YORN as FREE_GIFT_TARGET_YORN
