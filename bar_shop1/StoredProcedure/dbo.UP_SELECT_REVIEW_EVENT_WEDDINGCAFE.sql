IF OBJECT_ID (N'dbo.UP_SELECT_REVIEW_EVENT_WEDDINGCAFE', N'P') IS NOT NULL DROP PROCEDURE dbo.UP_SELECT_REVIEW_EVENT_WEDDINGCAFE
GO
...
-- 웨딩까페 이벤트 리스트 (2021.11.01) 
=================================================*/
CREATE PROCEDURE [dbo].[UP_SELECT_REVIEW_EVENT_WEDDINGCAFE]
...
BEGIN
	...
	IF @TXT_CARD_SEQ = 0 BEGIN
		...
		BEGIN
			...
			SET @SQL2 = @SQL2 + ' AND ER_REGDATE >= ''2022-08-30'' AND ER_USERID <> ''sheisymk'' AND ER_REVIEW_URL LIKE ''%cafe%'' '
			--SET @SQL2 = @SQL2 + ' AND ER_REGDATE >= ''2022-03-14'' AND ER_USERID <> ''sheisymk'' AND ER_REVIEW_URL LIKE ''%cafe%'' '
			EXEC (@SQL2)
			...
			SET @SQL = @SQL + ' JOIN COMPANY AS C ON A.ER_COMPANY_SEQ = C.COMPANY_SEQ '
				SET @SQL = @SQL + ' WHERE ER_TYPE='+CONVERT(VARCHAR(2),@ER_TYPE)+' AND ER_REGDATE >= ''2022-08-30'' AND ER_USERID <> ''sheisymk'' AND ER_REVIEW_URL LIKE ''%cafe%'' AND  ER_IDX NOT IN (SELECT TOP '+ CONVERT(VARCHAR(50), @PAGESIZE * (@PAGE - 1)) +' ER_IDX FROM S4_EVENT_REVIEW A WITH(NOLOCK) , COMPANY C  WHERE A.ER_COMPANY_SEQ = C.COMPANY_SEQ AND C.SALES_GUBUN  IN (''B'',''C'',''H'') AND ER_VIEW=0  ORDER BY '+@SORT_DESC+' DESC  ) '
		
			--SET @SQL = @SQL + ' WHERE ER_TYPE='+CONVERT(VARCHAR(2),@ER_TYPE)+' AND ER_REGDATE >= ''2022-03-14'' AND ER_USERID <> ''sheisymk'' AND ER_REVIEW_URL LIKE ''%cafe%'' AND  ER_IDX NOT IN (SELECT TOP '+ CONVERT(VARCHAR(50), @PAGESIZE * (@PAGE - 1)) +' ER_IDX FROM S4_EVENT_REVIEW A WITH(NOLOCK) , COMPANY C  WHERE A.ER_COMPANY_SEQ = C.COMPANY_SEQ AND C.SALES_GUBUN  IN (''B'',''C'',''H'') AND ER_VIEW=0  ORDER BY '+@SORT_DESC+' DESC  ) '
	...
	ELSE
		BEGIN
			...
			SET @SQL = @SQL + '	FIRST_UPLOAD_PHOTO = ( SELECT TOP 1 UPIMG_NAME  FROM S4_EVENT_REVIEW_PHOTO WHERE SEQ =  A.ER_IDX ORDER BY 1 ) '
			SET @SQL = @SQL + ' FROM S4_EVENT_REVIEW AS A WITH(NOLOCK) LEFT OUTER JOIN  S4_EVENT_REVIEW_STATUS AS B WITH(NOLOCK) ON A.ER_IDX = B.ERA_ER_IDX '
			SET @SQL = @SQL + ' WHERE ER_TYPE='+CONVERT(VARCHAR(2),@ER_TYPE)+' AND ER_REGDATE >= ''2022-08-30'' AND ER_REVIEW_URL LIKE ''%cafe%'' AND  ER_IDX NOT IN (SELECT TOP '+ CONVERT(VARCHAR(50), @PAGESIZE * (@PAGE - 1)) +' ER_IDX FROM S4_EVENT_REVIEW WITH(NOLOCK) '
		
			--SET @SQL = @SQL + ' WHERE ER_TYPE='+CONVERT(VARCHAR(2),@ER_TYPE)+' AND ER_REGDATE >= ''2022-03-14'' AND ER_REVIEW_URL LIKE ''%cafe%'' AND  ER_IDX NOT IN (SELECT TOP '+ CONVERT(VARCHAR(50), @PAGESIZE * (@PAGE - 1)) +' ER_IDX FROM S4_EVENT_REVIEW WITH(NOLOCK) '
