IF OBJECT_ID (N'dbo.SP_REPORT_SALES_LIST_TEST2', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_REPORT_SALES_LIST_TEST2
GO
...
CREATE PROCEDURE [dbo].[SP_REPORT_SALES_LIST_TEST2]
...
FROM (
	SELECT B.* 
	FROM (
		SELECT MAX(ORDER_COUPON_SEQ) AS ORDER_COUPON_SEQ
			, COUPON_ISSUE_SEQ
			, ORDER_SEQ
		FROM Custom_Order_Coupon 
		GROUP BY COUPON_ISSUE_SEQ, ORDER_SEQ
	) A 
	JOIN Custom_Order_Coupon B ON A.ORDER_COUPON_SEQ = B.ORDER_COUPON_SEQ 
) AS COC
INNER JOIN    Coupon_Issue AS CI ON COC.Coupon_Issue_Seq = CI.Coupon_Issue_Seq
INNER JOIN    Coupon_Detail AS CD ON CI.Coupon_Detail_Seq = CD.Coupon_Detail_Seq
INNER JOIN    Coupon_MST AS CM ON CD.Coupon_MST_Seq = CM.Coupon_MST_Seq
	...
	SELECT	
		...
		--,	CASE WHEN ISNULL(Coupon_Default.COUPON_AMT, 0)+ISNULL(Coupon_Dup.COUPON_AMT, 0) = 0 THEN ISNULL(CO.Reduce_Price, 0) ELSE ISNULL(Coupon_Default.COUPON_AMT, 0)*-1 END AS Reduce_Price
		...
		--,   CASE WHEN ISNULL(Coupon_Default.Coupon_Code, '') = '' THEN ISNULL(CO.CouponSeq, '')
		--			ELSE ISNULL(Coupon_Default.COUPON_NAME, '')		--ISNULL(Coupon_Default.Coupon_Code, '')
		--		END AS CouponSeq        
		--,   CASE WHEN ISNULL(Coupon_Dup.Coupon_Code, '') = '' THEN ISNULL(CO.Addition_CouponSeq, '')
		--			ELSE ISNULL(Coupon_Dup.COUPON_NAME, '')			--ISNULL(Coupon_Dup.Coupon_Code, '')
		--		END AS Addition_CouponSeq
		--,   ISNULL(Coupon_AD.COUPON_NAME, '') AS Coupon_Ad		--,   ISNULL(Coupon_AD.Coupon_Code, '') AS Coupon_Ad
		--,   ISNULL(Coupon_ADD.COUPON_NAME, '') AS Coupon_Add	--,   ISNULL(Coupon_ADD.Coupon_Code, '') AS Coupon_Add
		...
		BEGIN
			...
			FROM	#T AS T
                ...
                ( 
					SELECT  
                            CSO.Member_ID
                        ,   MAX(CSO.Request_Date) AS Request_Date
                        ,	STUFF
                            (
								(
									SELECT	
                ...
                ) AS CSO ON CSO.Member_ID = T.Member_ID
                ...
                (
                    ...
                    ON T.Member_ID = MS.UID
                LEFT OUTER JOIN
                (
                    SELECT 
                        uid
                    FROM EVENT_MARKETING_AGREEMENT
                    GROUP BY
                        uid
                ) AS MA
                    ON T.Member_ID = MA.UID
