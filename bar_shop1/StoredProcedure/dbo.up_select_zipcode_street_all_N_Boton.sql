IF OBJECT_ID (N'dbo.up_select_zipcode_street_all_N_Boton', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_zipcode_street_all_N_Boton
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[up_select_zipcode_street_all_N_Boton] (
			...
			if LEN(@jibun) > 0
				begin
					if  CHARINDEX('-', @jibun) <> 0	-- 지번정보가 있으면
						begin
							select top 1001 zipcode, sido, gungu, isnull(myoun, ''), street_name, build_no, build_sub_no, sigungu_build_name, isnull(b_name,b_ri) as b_name, b_ri, jibun_no, jibun_sub_no from zipcode_street with(nolock)
							where 
		...
		begin
			if LEN(@jibun) > 0
				begin
					if  CHARINDEX('-', @jibun) <> 0	-- 지번정보가 있으면
						begin
							select top 1001 zipcode, sido, gungu, isnull(myoun, ''), street_name, build_no, build_sub_no, sigungu_build_name, b_name, b_ri, jibun_no, jibun_sub_no from zipcode_street with(nolock)
							where 
