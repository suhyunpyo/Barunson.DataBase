USE [bar_shop1]
GO
/****** Object:  StoredProcedure [dbo].[SP_INSERT_INTEGRATION_MEMBER_SIGN_UP_FOR_EXIST_MEMBER_NEW_20230315]    Script Date: 2023-11-22 오후 3:12:27 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROC [dbo].[SP_INSERT_INTEGRATION_MEMBER_SIGN_UP_FOR_EXIST_MEMBER_NEW_20230315]  
...
--insert custom_order_history(htype, order_seq, admin_id, memo, system_sql, isVisible, ipAddress, reg_date)
-- values ('실가입2', 12332, @UID, '','',@CHK_LGMEMBERSHIP, @CHK_CASAMIAMEMBERSHIP, GETDATE())


 
insert custom_order_history(htype, order_seq, admin_id, memo, system_sql, isVisible, ipAddress, reg_date)
 values ('@P_AUTHCODE', 1111, @UID, @P_AUTHCODE,'','', '', GETDATE())
 
insert custom_order_history(htype, order_seq, admin_id, memo, system_sql, isVisible, ipAddress, reg_date)
 values ('@ADDRESS', 1111, @UID, @ADDRESS,'','', '', GETDATE())



	DECLARE @ReturnStr varchar(50)

    EXEC @ReturnStr = SP_INSERT_INTEGRATION_MEMBER_SIGN_UP_NEW_20230315   @P_AUTHCODE, @CONNINFO, @AUTHTYPE, @UID, @PWD, @UNAME, @EMAIL, @BIRTH, @BIRTH_DIV, 
