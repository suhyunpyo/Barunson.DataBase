IF OBJECT_ID (N'dbo.SP_EXEC_SAMPLE_REVIEW_COUPON_ISSUE_NEW_TEST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_EXEC_SAMPLE_REVIEW_COUPON_ISSUE_NEW_TEST
GO
...
*********************************************************************************************************************/ 
CREATE PROCEDURE [dbo].[SP_EXEC_SAMPLE_REVIEW_COUPON_ISSUE_NEW_TEST]  
 @P_IDX INT,   
 @P_VIEW INT,
 @P_STATUS INT,   
    ...
    */  
	...
	IF @UID = '' OR @UID IS NULL BEGIN --비회원에서 회원가입으로 전환 
		...
		FROM	S4_EVENT_REVIEW SER JOIN 
				...
				S2_USERINFO C ON C.UMAIL = SER.ER_EMAIL AND C.UNAME = SER.ER_USERNAME
