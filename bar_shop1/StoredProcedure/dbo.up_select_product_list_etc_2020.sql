IF OBJECT_ID (N'dbo.up_select_product_list_etc_2020', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_product_list_etc_2020
GO
...
/*
	...
	2019.12.29 단발성으로 인해 프로시저 새로 생성 HR
    2020 캘린더 /방명록에 같이 보여주세요

	up_select_product_list_etc_2020 @company_seq=5007,@card_div='c18',@page=1,@pagesize=10,@orderby='regdate',@sequence='asc'

	up_select_product_list_etc_2019 @company_seq=5007,@card_div='c18',@page=1,@pagesize=10,@orderby='regdate',@sequence='asc'
*/
-- =============================================
CREATE Procedure [dbo].[up_select_product_list_etc_2020]
...
BEGIN
	...
	IF (@card_div = 'C18') 
	BEGIN
		SET @card_div_list = @card_div_list + ',C13,C19'
	END 
	...
	(
		...
		WHERE 
			...
			--A.card_div=@card_div 
			--AND A.card_seq not in ('33958', '34420', '34431', '34432', '34836', '34880') -- 사용안하는 제품은 노출금지..daniel,kim
			AND A.card_seq not in ('33958', '34420', '34431', '34432', '34836', '34880','35387','35280') -- 사용안하는 제품은 노출금지 HR
			AND A.Card_Group='I' 
			AND B.isJumun='1'
			--AND B.isJumun='1' or ( A.card_seq in (37240,37241) and B.isJumun='1'
			AND B.Company_Seq=@company_seq
	) AS RESULT
