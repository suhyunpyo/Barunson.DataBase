...
/****** Object:  StoredProcedure [dbo].[SP_PP_MAIN_RANKING_LIST_V2]    Script Date: 4/28/2023 2:49:11 PM ******/
SET ANSI_NULLS ON
GO
...
-- =======================================================================================
CREATE PROCEDURE [dbo].[SP_PP_MAIN_RANKING_LIST_V2] 
...
BEGIN
	...
	IF @brand = 'I' OR @brand = 'N' --I : 인기상품 , N : 신상품
		BEGIN
			...
			set @parmDefinition_itm = N'@IN_company_seq int ,@IN_ordernum int ,@IN_image_wsize varchar(3), @IN_image_hsize varchar(3) , @IN_uid varchar(40)  , @IN_re_mincount int  , @IN_PRD_GB varchar(1) , @IN_V_PAGE int , @IN_V_PAGESIZE int , @IN_st INT'
			...
			ELSE
			BEGIN
				SET @strQuery = @strQuery + '										,''N'' Wishcard'+char(13) + char(10)
	            SET @strQuery = @strQuery + '										,ISNULL((SELECT top 1  ''Y'' FROM s2_samplebasket WHERE COMPANY_SEQ = 5003 AND CARD_SEQ = B.CARD_SEQ AND GUID=@IN_uid and guid <> '''') , ''N'') sampleCnt'+char(13) + char(10)
			END
		...
		BEGIN
			...
			set @parmDefinition_itm = N'@IN_company_seq int ,@IN_ordernum int ,@IN_image_wsize varchar(3), @IN_image_hsize varchar(3) , @IN_uid varchar(40)  , @IN_re_mincount int , @IN_st INT , @IN_V_PAGE int , @IN_V_PAGESIZE int'
			...
			begin
				SET @strQuery = @strQuery + '	''N'' WISHCARD,'+char(13) + char(10)	
		        SET @strQuery = @strQuery + '	ISNULL((SELECT top 1  ''Y'' FROM s2_samplebasket WHERE CARD_SEQ = A.RK_CARD_CODE AND COMPANY_SEQ = 5003 AND GUID=@IN_uid and guid <> '''') , ''N'') sampleCnt ,'+char(13) + char(10)	
			end
...
--            SET @strQuery = @strQuery + '								,ISNULL((SELECT top 1  ''Y'' FROM s2_samplebasket WHERE COMPANY_SEQ = 5003 AND CARD_SEQ = B.CARD_SEQ AND COMPANY_SEQ = 5003 AND UID=@IN_uid and uid <> '''') , ''N'') sampleCnt'+char(13) + char(10)	
	...
	ELSE	--H : 고가상품 , L : 저가상품
		BEGIN
			...
			set @parmDefinition_itm = N'@IN_company_seq int ,@IN_ordernum int ,@IN_image_wsize varchar(3), @IN_image_hsize varchar(3) , @IN_uid varchar(40)  , @IN_re_mincount int '
			...
			begin
				SET @strQuery = @strQuery + '						''N'' Wishcard,' +char(13) + char(10)
		        SET @strQuery = @strQuery + '						ISNULL((SELECT top 1  ''Y'' FROM s2_samplebasket WHERE CARD_SEQ = B.CARD_SEQ AND COMPANY_SEQ = 5003 AND GUID=@IN_uid and guid <> '''') , ''N'') sampleCnt' +char(13) + char(10)
			end
