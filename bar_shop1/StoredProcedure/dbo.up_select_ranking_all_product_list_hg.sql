IF OBJECT_ID (N'dbo.up_select_ranking_all_product_list_hg', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_ranking_all_product_list_hg
GO
...
-- exec up_select_ranking_all_product_list_hg 5007,NULL,1,200,'REGDATE','DESC',400,NULL,200,2000,1,2,3,'0','G1','G2','G3','S1','S2','S3','S4',372,373,374,375,376,377,457,460,458,462

-- =============================================
CREATE PROCEDURE [dbo].[up_select_ranking_all_product_list_hg]
...
BEGIN
	...
	*/
	...
	--DECLARE @page			int			= 1;
	--DECLARE @pagesize		int			= 48;
	--DECLARE @orderby		nvarchar(20)	= 'SAMPLE';
	--DECLARE @Sequence		nvarchar(20)	= 'DESC';
	--DECLARE @order_num	int			= 300;
	--DECLARE @keyword		nvarchar(20)	= '';
	--DECLARE @price1		int			= 0;
	--DECLARE @price2		int			= 3000;
	--DECLARE @shape1		int				= 1;
	--DECLARE @shape2		int				= 2;
	--DECLARE @shape3		int				= 3;
	--DECLARE @folding1		varchar(2)	= '0';
	--DECLARE @folding2		varchar(2)	= 'G1';
	--DECLARE @folding3		varchar(2)	= 'G2';
	--DECLARE @folding4		varchar(2)	= 'G3';
	--DECLARE @folding5		varchar(2)	= 'S1';
	--DECLARE @folding6		varchar(2)	= 'S2';
	--DECLARE @folding7		varchar(2)	= 'S3';
	--DECLARE @folding8		varchar(2)	= 'E0';
	--DECLARE @style1			int			= 372;
	--DECLARE @style2			int			= 373;
	--DECLARE @style3			int			= 374;
	--DECLARE @style4			int			= 375;
	--DECLARE @style5			int			= 376;
	--DECLARE @style6			int			= 377;
	--DECLARE @print1			int			= 457;
	--DECLARE @print2			int			= 460;
	--DECLARE @print3			int			= 458;
	--DECLARE @print4			int			= 462;
	...
	--SET @strQuery = @strQuery + ' 						SELECT Card_Seq, COUNT(*) AS Cnt 																				     '+char(13) + char(10)
	--SET @strQuery = @strQuery + ' 						FROM CUSTOM_SAMPLE_ORDER_ITEM  																	     '+char(13) + char(10)
	--SET @strQuery = @strQuery + ' 						WHERE Reg_Date BETWEEN @startDate AND @endDate															     '+char(13) + char(10)
	--SET @strQuery = @strQuery + ' 						GROUP BY Card_Seq																							     '+char(13) + char(10)
	...
	SET @strQuery = @strQuery + ' 	  AND ( A.Card_Name LIKE ''%'' + ISNULL(@keyword, '''') + ''%'' OR A.Card_Code LIKE ''%'' + ISNULL(@keyword, '''') + ''%'' )		'+char(13) + char(10)
	...
	SET @strQuery = @strQuery + ' WHERE RowNum BETWEEN ( ( (1 - 1) * 48 ) + 1 ) AND ( 1 * 48 )					'+char(13) + char(10)
	...
	exec sp_executesql @strQuery , @parmDefinition_itm ,@company_seq  , @brand , @page  , @pagesize  , @orderby  , @Sequence  , @order_num  , @keyword  , @price1  , @price2  , @shape1  , @shape2  , @shape3  , @folding1	 , @folding2	 , @folding3	 , @folding4	 ,@folding5	 ,@folding6	 ,@folding7	 ,@folding8	 ,@style1  ,@style2  ,@style3  ,@style4  ,@style5  ,@style6  , @print1  , @print2  , @print3  , @print4 ,@startDate ,@endDate
