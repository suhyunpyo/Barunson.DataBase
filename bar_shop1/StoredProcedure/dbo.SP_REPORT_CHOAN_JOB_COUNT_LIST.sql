IF OBJECT_ID (N'dbo.SP_REPORT_CHOAN_JOB_COUNT_LIST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_REPORT_CHOAN_JOB_COUNT_LIST
GO
...
-- EXEC [SP_REPORT_CHOAN_JOB_COUNT_LIST] '2016-01-01', '2016-12-31', 'N'

CREATE PROCEDURE [dbo].[SP_REPORT_CHOAN_JOB_COUNT_LIST]
...
BEGIN
	...
	SELECT  SUM(A.C_DEFAULT)			AS [Default]
		,	SUM(A.C_SPECIAL)			AS [Special]
		,   SUM(A.C_ADD)				AS [Add]
		,   SUM(A.C_ADD_SPECIAL)		AS [AddSpecial]
		,   SUM(A.C_TOT)				AS [Total]

		,   SUM(A.CMOD_DEFAULT)			AS [ModifyDefault]
		,   SUM(A.CMOD_SPECIAL)			AS [ModifySpecial]
		,   SUM(A.CMOD_ADD)				AS [ModifyAdd]
		,   SUM(A.CMOD_ADD_SPECIAL)		AS [ModifyAddSpecial]
		,   SUM(A.CMOD_TOT)				AS [ModifyTotal]

		,	SUM(A.WEDD_TOT)				AS [MapTotal]

		,   ISNULL((SELECT ADMIN_NAME + '(' + A.ADMIN_ID + ')' FROM ADMIN_LST WHERE ADMIN_ID = A.ADMIN_ID), '합계')	AS [ID]
