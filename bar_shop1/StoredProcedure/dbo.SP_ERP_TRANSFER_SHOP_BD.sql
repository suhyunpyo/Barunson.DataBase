IF OBJECT_ID (N'dbo.SP_ERP_TRANSFER_SHOP_BD', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_ERP_TRANSFER_SHOP_BD
GO
...
GO
CREATE PROCEDURE [dbo].[SP_ERP_TRANSFER_SHOP_BD]
	@SDate as char(8)
	, @EDate as char(8)
...
-- EXEC SP_ERP_TRANSFER_SHOP_BD '20190111', '20190111'
...
WHERE A.status_Seq = 15  -- 배송완료
	...
	and convert(char(8), A.Order_date,112) >= '20190101'
...
-- 배송비는 안올림
	...
	SELECT 
		...
		b_goodGubun	= 'SO',
		b_seq		= 1,
		b_storeCode	= 'DF05',	--DF05:샘플전용창고(4층)
...
WHERE Convert(char(8), a.delivery_Date,112) BETWEEN @SDate AND @EDate
	...
	JOIN (
			...
			FROM (
					...
					WHERE  Convert(char(8),delivery_date,112) between @SDate AND @EDate
						and a.status_seq >= 12 
						and a.order_type in ('5','2','F','P', 'C', 'G', 'S', 'D','K', 'B', 'R', 'Q', 'J', 'H', 'N', 'E','T','O','B','M', 'X', 'U', 'Z')--송월타월'T',손보자기'O',비스트디자인'B',셀레모기획전'M' 비핸즈 답례품 추가20171013 유우종  --속지봉투:E 추가 20161102 이상민
			...
			WHERE Convert(char(8),delivery_date,112) between @SDate AND @EDate
				...
				and status_seq >= 12 
				and order_type in ('5','2','F','P', 'C', 'G', 'S', 'D','K', 'B', 'R', 'Q', 'J', 'H', 'N', 'E','T','O','B','M', 'X', 'U', 'Z')--송월타월'T',손보자기'O',비스트디자인'B',셀레모기획전'M' 비핸즈 답례품 추가20171013 유우종
		...
		--JOIN #CardMaster d ON b.card_seq = d.card_seq
		LEFT JOIN #sales_gubunTemp O ON a.sales_gubun = O.sales_gubun
		LEFT JOIN [erpdb.bhandscard.com].BHC.DBO.itemSiteMaster E ON E.SiteCode = 'BHC2' AND b.Card_ErpCode = E.ItemCode

	WHERE Convert(char(8),a.delivery_date,112) between @SDate AND @EDate
		and a.status_seq >= 12 
		and a.order_type in ('5','2','F','P', 'C', 'G', 'S', 'D','K', 'B', 'R', 'Q', 'J', 'H', 'N', 'E','T','O','B','M', 'X', 'U', 'Z')--송월타월'T',손보자기'O',비스트디자인'B',셀레모기획전'M' 비핸즈 답례품 추가20171013 유우종
