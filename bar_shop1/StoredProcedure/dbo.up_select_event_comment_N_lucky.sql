IF OBJECT_ID (N'dbo.up_select_event_comment_N_lucky', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_event_comment_N_lucky
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[up_select_event_comment_N_lucky]
...
BEGIN
	...
	(
		SELECT  ROW_NUMBER() OVER (ORDER BY A.ER_Idx DESC ) AS RowNum				
			...
			, ( SELECT top 1 ts.CouponCD FROM tCouponSub TS WHERE TS.USERID = A.ER_UserId AND TS.COUPONCD IN ('C0000190', 'C0000191', 'C0000192', 'C0000193', 'C0000194') AND CONVERT(CHAR(10),TS.TakeDT) = CONVERT(CHAR(10),A.ER_Regdate)) AS couponCD
		FROM 
		...
		WHERE 
			...
			AND A.ER_card_seq = ISNULL(88, A.ER_card_seq)
