IF OBJECT_ID (N'dbo.SP_EVENT_BARUNSON_NEWCARD_VOTE_MAX_v2', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_EVENT_BARUNSON_NEWCARD_VOTE_MAX_v2
GO
...
-- EXEC dbo.[SP_EVENT_BARUNSON_NEWCARD_VOTE_MAX] '아이디', '상품코드', '투표코드'  , '최대클릭수'  , '쿠폰코드'  

-- 20년도에 이벤트 살짝 변경됨
-- =============================================  
  
CREATE PROCEDURE [dbo].[SP_EVENT_BARUNSON_NEWCARD_VOTE_MAX_v2]  
 @UID        AS VARCHAR(50),  
 @GUID        AS VARCHAR(100),  
...
BEGIN  
 ...
 -- 이미 투표를 했는 지 확인  
	...
	FROM S2_CARD C, S4_POLL_ITEM P
	WHERE C.CARD_CODE = P.ITEM_TITLE
	AND SEQ = @POLL_ITEM_SEQ
 ...
 BEGIN  
  ...
  -- (추가) 요기 LIKE 연동.
  EXEC SP_INSERT_S2_CARD_LIKE @CARD_SEQ, @UID, @GUID,  @COMPANY_SEQ, @SALES_GUBUN
  ...
  IF @DAY_COUNT = (@DAY_MAX-1) AND NOT EXISTS(  
      ...
      AND   CD.COUPON_CODE = @COUPON_CODE  
	   ...
	   BEGIN  
        ...
        SET @RESULT_MESSAGE = '쿠폰이 발급되었습니다. :)'  
          
	   END  
    ELSE  
        BEGIN  
        SET @RESULT_CODE = '0000'  
        SET @RESULT_MESSAGE = '3종의 제품에 투표 완료하면 쿠폰이 발급됩니다.'  
        END  
    END  
ELSE  
    BEGIN  
