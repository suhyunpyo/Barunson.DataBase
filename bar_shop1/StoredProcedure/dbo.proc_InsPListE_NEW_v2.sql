IF OBJECT_ID (N'dbo.proc_InsPListE_NEW_v2', N'P') IS NOT NULL DROP PROCEDURE dbo.proc_InsPListE_NEW_v2
GO
...
GO
CREATE    Procedure [dbo].[proc_InsPListE_NEW_v2]   
 @order_seq int,   
 @title varchar(50),   
 @card_seq int,   
 @ptype char(1),   
 ...
 @EnvSpecialType as varchar(2) = null,  
 @pid int output   
 as   
 begin   
    ...
    BEGIN
      ...
      ,isZipBox,recv_tail, isNotPrint_Addr,EnvSpecialType) values(@order_seq,@ptype,@card_seq,@title,@pcount,@isNotPrint  
      ,@env_person1,@env_person2,@env_person_tail,@env_person1_tail,@env_person2_tail,@isenv_person_tail  
      ,@env_phone,@env_hphone,@env_hphone2,@env_zip,@env_addr,@env_addr_detail,@isPostMark,@PostName,@PostTail  
      ,@isZipBox,@recv_tail, @isNotPrint_Addr,ISNULL(@EnvSpecialType,NULL))  
    END
    ELSE
    BEGIN
      insert into CUSTOM_ORDER_PLIST(order_seq,print_type,card_seq,title,print_count,isNotPrint  
