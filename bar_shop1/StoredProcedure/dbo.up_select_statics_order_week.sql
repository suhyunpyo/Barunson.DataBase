IF OBJECT_ID (N'dbo.up_select_statics_order_week', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_statics_order_week
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[up_select_statics_order_week]
...
from
(
    select  convert(varchar(10),order_date,121) AS SDD, left(CONVERT(VARCHAR, order_date, 8),2) AS STT, count(order_seq) AS cnt
    from    custom_order with(nolock)
    where   convert(varchar(10),order_date,121) between convert(varchar(10),DATEADD(DD, -10, GETDATE()),121) and convert(varchar(10),GETDATE(),121)
    --and company_seq in ('5001', '5006', '5007', '5003')  
    group by convert(varchar(10),order_date,121),
