IF OBJECT_ID (N'dbo.SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_NEW_DELIVERY_STOP_TEST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_NEW_DELIVERY_STOP_TEST
GO
...
CREATE PROCEDURE [dbo].[SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_NEW_DELIVERY_STOP_TEST]
	...
	SET @SEARCH_DATE_END_OF_DAY = CAST(@P_SEARCH_END_DATE + ' ' + @P_SEARCH_END_HOUR + ':59:59' AS DATETIME)
		...
		SELECT	1 AS STATUS
			...
			,   CASE 
				...
				SITENAME = '샘플(' + K.CODE_VALUE + ')',
				 롯데택배송장번호  = (
										select ISNULL(DELIVERY_CODE_NUM, '') 
										from CUSTOM_SAMPLE_ORDER  
										where sample_order_seq = CSO.SAMPLE_ORDER_SEQ and  delivery_com = 'LT' 
									  )
		...
		LEFT OUTER JOIN manage_code K  ON CSO.SALES_GUBUN = K.code AND K.code_type = 'sales_gubun' 
		WHERE	1 = 1
		AND		H.GUBUN = 'B'
