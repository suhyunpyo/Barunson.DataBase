IF OBJECT_ID (N'dbo.up_select_review_event_bhands', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_review_event_bhands
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[up_select_review_event_bhands]
...
BEGIN
	...
	if @txt_card_seq = 0
		begin
			set @SQL2 = ' select count(ER_Idx) from S4_Event_Review with(nolock) where ER_Type='+CONVERT(VARCHAR(2),@ER_Type)+' and ER_Company_Seq='+CONVERT(VARCHAR(50),@company_seq)+' and ER_View=0 and er_regdate >= ''2020-01-01'' '
			exec (@SQL2)
			...
			set @SQL = @SQL + ' and ER_Company_Seq ='+CONVERT(VARCHAR(50),@company_seq)+'  and ER_View=0 and er_regdate >= ''2020-01-01'' order by '+@sort_desc+' desc'
