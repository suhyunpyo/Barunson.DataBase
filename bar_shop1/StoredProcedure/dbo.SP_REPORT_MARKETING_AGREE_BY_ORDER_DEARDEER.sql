IF OBJECT_ID (N'dbo.SP_REPORT_MARKETING_AGREE_BY_ORDER_DEARDEER', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_REPORT_MARKETING_AGREE_BY_ORDER_DEARDEER
GO
...
GO
/****************************************************************************************************************
-- SP Name       : SP_REPORT_MARKETING_AGREE_BY_ORDER_DEARDEER
-- Author        : 박혜림
-- Create date   : 2022-09-01
-- Description   : 디얼디어 마케팅 동의 통계(결제완료일)
-- Update History: 
-- Comment       : SP_REPORT_MARKETING_AGREE_BY_ORDER 참고
--
--
-- EXEC SP_REPORT_MARKETING_AGREE_BY_ORDER_DEARDEER '2022-09-08','2022-09-15'
--
****************************************************************************************************************/
CREATE PROCEDURE [dbo].[SP_REPORT_MARKETING_AGREE_BY_ORDER_DEARDEER]  
...
BEGIN  
	...
	-- 주문건 추출
	-------------------------------------------------------
	SELECT DISTINCT sales_Gubun AS Sales_Gubun
	     , T1.member_id AS member_id
	  INTO #TT
	  FROM custom_order              AS T1 WITH(NOLOCK)
	 ...
	 WHERE T1.sales_Gubun = 'SD'
	   ...
	   AND (T1.settle_date IS NOT NULL AND T1.settle_date >= @p_start_date + ' 00:00:00' AND T1.settle_date <= @p_end_date + ' 23:59:59')
	...
	SELECT DISTINCT 'SD' AS Sales_Gubun
	 ...
	 INNER JOIN S2_UserInfo_Deardeer_Marketing_View AS T2 WITH(NOLOCK) ON (T1.uid = T2.uid AND T2.agreement_step = 'O')
