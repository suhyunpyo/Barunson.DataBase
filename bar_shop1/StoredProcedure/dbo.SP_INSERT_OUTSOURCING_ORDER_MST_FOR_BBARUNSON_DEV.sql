IF OBJECT_ID (N'dbo.SP_INSERT_OUTSOURCING_ORDER_MST_FOR_BBARUNSON_DEV', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_INSERT_OUTSOURCING_ORDER_MST_FOR_BBARUNSON_DEV
GO
...
EXEC SP_INSERT_OUTSOURCING_ORDER_MST_FOR_BBARUNSON 2143980, '관리자', '107001'  
 ...
 SP_INSERT_OUTSOURCING_ORDER_MST_FOR_BBARUNSON 3014642, '관리자', '107014', NULL
 SP_INSERT_OUTSOURCING_ORDER_MST_FOR_BBARUNSON 3014642, '관리자', '107014', NULL, 37398
 SP_INSERT_OUTSOURCING_ORDER_MST_FOR_BBARUNSON 3014642, '관리자', '107014', NULL, 37399
*/  

CREATE PROCEDURE [dbo].[SP_INSERT_OUTSOURCING_ORDER_MST_FOR_BBARUNSON_DEV]  
    @P_ORDER_SEQ                  AS INT  
,   @P_REQUESTOR_NAME             AS NVARCHAR(100)  
,   @P_COMPANY_TYPE_CODE          AS VARCHAR(6)  
,   @PDF_CARDCODE				  AS VARCHAR(20)  
,   @P_CARD_SEQ				      AS INT  = NULL
...
BEGIN  
	...
	/*
		...
		WHERE   CO.ORDER_SEQ = @P_ORDER_SEQ  
	*/
		IF @P_COMPANY_TYPE_CODE = '107008' OR @P_COMPANY_TYPE_CODE = '107017'		/*태산인 경우 사고건 포함하여 각각의 껀이 넘어올 수 있음 2020.05.20 김수경 추가. 포켓카드 경우 추가(2021-08-13) */
		BEGIN
		...
		ELSE
		BEGIN
		/* 부속 카드만 주문시 주문수량 NULL 넘어 오는 부분 수정 JOIN 조건의 카드 번호 조건 삭제 */
			SELECT  @ORDER_QTY = MAX(COI.item_count)  
			FROM    CUSTOM_ORDER CO  
			INNER JOIN    CUSTOM_ORDER_ITEM COI ON CO.ORDER_SEQ = COI.ORDER_SEQ
			WHERE   CO.ORDER_SEQ = @P_ORDER_SEQ		  
		END
	END  
	...
	END
	-- 내부 디지털 건만 처리 되게 수정 (2020-07-21)
	ELSE IF @P_COMPANY_TYPE_CODE = '107012' OR @P_COMPANY_TYPE_CODE = '107014' 
	BEGIN
		IF EXISTS ( 
			SELECT B.card_Code 
		...
		END 
	END
	ELSE IF @P_COMPANY_TYPE_CODE = '107017' -- 포켓카드
	BEGIN
		IF EXISTS ( 
			SELECT B.card_Code 
			FROM custom_order_plist A JOIN S2_Card B ON A.card_seq = B.Card_Seq 
	...
	ELSE
	BEGIN
		SELECT @CARD_CODE AS CARD_CODE FROM S2_Card WHERE CARD_SEQ = @CARD_SEQ;
	END


	/*@P_CARD_SEQ*/
	IF @P_CARD_SEQ IS NOT NULL
	BEGIN
		SET @CARD_SEQ = @P_CARD_SEQ
	END
	...
	/* ERP 정보 셋팅 */  
	-- ERP_CARD_CODE와 CARD_CODE 같이 검색되게 수정 (2020-07-21)
	IF (EXISTS(SELECT * FROM [erpdb.bhandscard.com].[XERP].dbo.VW_CARD_PRINT_INFO_WITH_WEPOD WHERE (CARD_CODE = @ERP_CARD_CODE OR CARD_CODE = @CARD_CODE) ))  
	BEGIN  
		SELECT  TOP 1  
		...
		WHERE   (CARD_CODE = @ERP_CARD_CODE OR CARD_CODE = @CARD_CODE) 
	...
	SELECT @PRINT_FILE_NAME_ORIGINAL = CASE @P_COMPANY_TYPE_CODE 
											...
											WHEN '107016' THEN '\Kwangyil' -- 광일 #5945 외주 가공처 변경 (금박 :  현대 금박 추가)
											WHEN '107017' THEN '\Pocket' -- 포켓카드 추가
	...
	/* 포켓카드(디지털내부) */
	IF ( @P_COMPANY_TYPE_CODE = '107017')
	BEGIN  
		SET @PRINT_FILE_NAME_DOWNLOAD = '포켓카드_' + CAST(@P_ORDER_SEQ AS VARCHAR(100)) + '_' + @PAPER_TYPE_NAME  
		SET @PRINT_FILE_NAME_DOWNLOAD = @PRINT_FILE_NAME_DOWNLOAD + '_조판수(_' + CAST(FLOOR(@PAGES_PER_SHEET_VALUE) AS VARCHAR(10)) + '_)'  
		SET @PRINT_FILE_NAME_DOWNLOAD = @PRINT_FILE_NAME_DOWNLOAD + '_주문수량(_' + CAST(FLOOR(@ORDER_QTY) AS VARCHAR(10)) + '_)'  
		SET @PRINT_FILE_NAME_DOWNLOAD = @PRINT_FILE_NAME_DOWNLOAD + '_' + LTRIM(RTRIM(@CARD_CODE)) + '.pdf'  
	END  
