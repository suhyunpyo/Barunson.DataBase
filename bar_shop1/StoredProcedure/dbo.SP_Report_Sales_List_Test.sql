IF OBJECT_ID (N'dbo.SP_Report_Sales_List_Test', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_Report_Sales_List_Test
GO
...
/*  
 EXEC SP_REPORT_SALES_LIST    'AGGREGATE'  , ' SB , '  , ''  ,  0 , '2021-04-01'  , '2021-04-22'  ,  0 ,  0 ,  0 ,  -1 ,  0 ,  0 , ''  , 'N'  , 'N'  , '' 
  EXEC SP_REPORT_SALES_LIST    'LIST'  , ' SB , '  , ''  ,  0 , '2021-04-01'  , '2021-04-22'  ,  0 ,  0 ,  0 ,  -1 ,  0 ,  0 , ''  , 'N'  , 'N'  , '' 
*/  
CREATE PROCEDURE [dbo].[SP_Report_Sales_List_Test]  
...
select * into #custom_order_year from custom_order AS CO with(nolock) 
where 1=1
...
*/
 ...
 SELECT   
  ...
  ,   CASE WHEN ISNULL(dbo.FN_ORDER_COUPON_LIST(CO.ORDER_SEQ,'131001'),'') <> '' THEN dbo.FN_ORDER_COUPON_LIST(CO.ORDER_SEQ,'131001')  
   ELSE  
    CASE WHEN ISNULL(LTRIM(Coupon_Default.COUPON_NAME), '') <> '' THEN ISNULL(LTRIM(RTRIM(Coupon_Default.COUPON_NAME)), '')  
     ELSE ( SELECT TOP 1 ISNULL(coupon_desc, CO.CouponSeq) AS COUPON_NAME FROM S4_COUPON WHERE COUPON_CODE = ISNULL(Coupon_Default.Coupon_Code, CO.CouponSeq ))  
    END   
   END AS CouponSeq  
  
  ,   CASE WHEN ISNULL(dbo.FN_ORDER_COUPON_LIST(CO.ORDER_SEQ,'131002'),'') <> '' THEN dbo.FN_ORDER_COUPON_LIST(CO.ORDER_SEQ,'131002')  
   ELSE  
    CASE WHEN ISNULL(LTRIM(Coupon_Dup.COUPON_NAME), '') <> '' THEN ISNULL(LTRIM(RTRIM(Coupon_Dup.COUPON_NAME)), '')  
     ELSE ( SELECT TOP 1 ISNULL(coupon_desc, CO.Addition_CouponSeq) AS COUPON_NAME FROM S4_COUPON WHERE COUPON_CODE = ISNULL(Coupon_Dup.Coupon_Code, CO.Addition_CouponSeq ))  
    END   
   END AS Addition_CouponSeq  
  
  ,   CASE WHEN ISNULL(dbo.FN_ORDER_COUPON_LIST(CO.ORDER_SEQ,'131003'),'') <> '' THEN dbo.FN_ORDER_COUPON_LIST(CO.ORDER_SEQ,'131003')  
   ELSE  
    CASE WHEN ISNULL(LTRIM(Coupon_AD.COUPON_NAME), '') <> '' THEN ISNULL(LTRIM(RTRIM(Coupon_AD.COUPON_NAME)), '')  
     ELSE ( SELECT TOP 1 ISNULL(coupon_desc, Coupon_AD.Coupon_Code) AS COUPON_NAME FROM S4_COUPON WHERE COUPON_CODE = Coupon_AD.Coupon_Code )  
    END   
   END AS Coupon_Ad  
  
  ,   CASE WHEN ISNULL(dbo.FN_ORDER_COUPON_LIST(CO.ORDER_SEQ,'131004'),'') <> '' THEN dbo.FN_ORDER_COUPON_LIST(CO.ORDER_SEQ,'131004')  
   ELSE  
    CASE WHEN ISNULL(LTRIM(Coupon_ADD.COUPON_NAME), '') <> '' THEN ISNULL(LTRIM(RTRIM(Coupon_ADD.COUPON_NAME)), '')  
     ELSE ( SELECT TOP 1 ISNULL(coupon_desc, Coupon_ADD.Coupon_Code) AS COUPON_NAME FROM S4_COUPON WHERE COUPON_CODE = Coupon_ADD.Coupon_Code )  
    END   
   END AS Coupon_Add  
...
*  
 ...
 ELSE IF @P_List_Type = 'List_Excel'  
  ...
  JOIN #T AS T_SUB ON CWO.Member_ID = T_SUB.Member_ID  
  ...
  GROUP BY CWO.Member_ID
   ...
   FROM #T AS T  
		  ...
		  LEFT OUTER JOIN  
		   (  
				...
				FROM  BARUNSON.DBO.TB_ORDER A INNER JOIN 
					  BARUNSON.DBO.TB_ORDER_PRODUCT B ON A.ORDER_ID = B.ORDER_ID INNER JOIN 
					  BARUNSON.DBO.TB_PRODUCT C ON B.PRODUCT_ID = C.PRODUCT_ID
	  ...
	  --  LEFT OUTER JOIN  
		...
		LEFT OUTER JOIN ( SELECT uid FROM S2_USERINFO_THIRD_PARTY_MARKETING_AGREEMENT  WITH(NOLOCK) GROUP BY uid ) AS MS ON T.Member_ID = MS.UID  
		LEFT OUTER JOIN ( SELECT uid FROM EVENT_MARKETING_AGREEMENT  WITH(NOLOCK) GROUP BY uid ) AS MA ON T.Member_ID = MA.UID  
