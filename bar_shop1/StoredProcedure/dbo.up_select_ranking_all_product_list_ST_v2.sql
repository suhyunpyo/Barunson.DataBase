IF OBJECT_ID (N'dbo.up_select_ranking_all_product_list_ST_v2', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_ranking_all_product_list_ST_v2
GO
...
-- 2020.02.25 테스트버전으로 만들기        
-- =============================================        
CREATE PROCEDURE [dbo].[up_select_ranking_all_product_list_ST_v2]        
     ...
     @print4      int,     -- print4        
     --@discRate  int     -- 할인율        
     @orderby2  nvarchar(20),  -- 정렬 컬럼        
     @Sequence2  nvarchar(20)  -- 정렬 조건(ASC, DESC)        
AS        
BEGIN        
...
FROM    (  
 ...
 WHERE 1 = 1        
   ...
   AND (J.CardKind_Seq in  (1,6,17) OR (ISNULL(@keyword, '') <> '' AND J.CardKind_Seq = 3)) -- (청첩장, 답례장) OR (검색어 있을때만 감사장 포함)         
   AND ( A.Card_Name LIKE '%' + ISNULL(@keyword, '') + '%' OR A.Card_Code LIKE '%' + ISNULL(@keyword, '') + '%' OR  Replace(Replace(A.Card_Name,' ',''),CHAR(10),'') LIKE '%' + ISNULL(@keyword, '') + '%' OR Replace(Replace(A.Card_Code,' ',''),CHAR(10),'')
 ...
 LIKE '%' + ISNULL(@keyword, '') + '%')      
   ...
   AND CD.Card_Folding IN (@folding1, @folding2, @folding3, @folding4, @folding5, @folding6, @folding7, @folding8, 'FS')                   
 -- Count Query 끝 --         
         
                 
 set @parmDefinition_itm = N'@company_seq int , @brand varchar(20), @page INT , @pagesize INT , @orderby varchar(20) , @Sequence varchar(20) ,@order_num INT , @keyword varchar(20) , @price1 INT , @price2 int ,@shape1 int ,@shape2 int ,@shape3 int ,@folding1 varchar(2) ,@folding2 varchar(2) ,@folding3 varchar(2) ,@folding4 varchar(2) ,@folding5 varchar(2) ,@folding6 varchar(2) ,@folding7 varchar(2) ,@folding8 varchar(2) ,@style1 INT ,@style2 INT ,@style3 INT ,@style4 INT ,@style5 INT ,@style6 INT , @print1
 INT , @print2 INT , @print3 INT , @print4 INT ,@startDate datetime , @endDate datetime, @orderby2 varchar(20) , @Sequence2 varchar(20)'        
 ...
 SET @strQuery = @strQuery + '            CASE @Sequence2 WHEN ''ASC'' THEN                    '+char(13) + char(10)        
 SET @strQuery = @strQuery + '             CASE @orderby2 WHEN ''REGDATE'' THEN A.RegDate               '+char(13) + char(10)        
 ...
 SET @strQuery = @strQuery + '            (                            '+char(13) + char(10)        
 SET @strQuery = @strQuery + '            CASE @Sequence2 WHEN ''DESC'' THEN                    '+char(13) + char(10)        
 SET @strQuery = @strQuery + '             CASE @orderby2 WHEN ''REGDATE'' THEN A.RegDate               '+char(13) + char(10)        
 SET @strQuery = @strQuery + '              WHEN ''PRICE'' THEN (A.CardSet_Price * @order_num) * ((100-Discount_Rate)/100)        '+char(13) + char(10)        
 ...
 SET @strQuery = @strQuery + '             END                          '+char(13) + char(10)        
 SET @strQuery = @strQuery + '            END                           '+char(13) + char(10)        
 SET @strQuery = @strQuery + '            ) DESC,                           '+char(13) + char(10)        
 SET @strQuery = @strQuery + '            (                     '+char(13) + char(10)        
 ...
 SET @strQuery = @strQuery + '    , ISNULL(SC.SAMPLE_RNK,''0'') AS SAMPLE_RNK                             '+char(13) + char(10)        
 SET @strQuery = @strQuery + '    , ISNULL(cd.Minimum_Count,''100'') AS min_count                             '+char(13) + char(10)        
 SET @strQuery = @strQuery + '    , ISNULL((SELECT COUPON_MST_SEQ FROM COUPON_MST WHERE COUPON_NAME = ''SUPER_DAYS_''+A.Card_Code AND COUPON_ISSUE_END_DATE  > GETDATE()),0) AS COUPON_MST_SEQ'+char(13) + char(10)        
 SET @strQuery = @strQuery + '    , ISNULL((SELECT CARD_SEQ FROM S4_MD_CHOICE WHERE MD_SEQ = 752 AND CARD_SEQ = A.CARD_SEQ ),0) AS EVENT_CARD '+char(13) + char(10)  
 ...
 SET @strQuery = @strQuery + '  LEFT OUTER JOIN (                                 '+char(13) + char(10)        
 SET @strQuery = @strQuery + '    SELECT Card_Seq , Cnt, SAMPLE_RNK  FROM #CUSTOM_SAMPLE_ORDER_ITEM_CNT              '+char(13) + char(10)              
 ...
 SET @strQuery = @strQuery + '    AND (J.CardKind_Seq in (1,6,17) OR (ISNULL(@keyword, '''') <> '''' AND J.CardKind_Seq = 3)) -- (청첩장, 답례장) OR (검색어 있을때만 감사장 포함)    '+char(13) + char(10)        
 ...
 SET @strQuery = @strQuery + '    AND CD.Card_Folding IN (@folding1, @folding2, @folding3, @folding4, @folding5, @folding6, @folding7, @folding8,''FS'') '+char(13) + char(10)        
...
,@folding5  ,@folding6  ,@folding7  ,@folding8  ,@style1  ,@style2  ,@style3  ,@style4  ,@style5  ,@style6  , @print1  , @print2  , @print3  , @print4 ,@startDate ,@endDate    , @orderby2  , @Sequence2    
