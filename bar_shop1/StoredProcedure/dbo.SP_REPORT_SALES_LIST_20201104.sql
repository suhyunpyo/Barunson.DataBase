IF OBJECT_ID (N'dbo.SP_REPORT_SALES_LIST_20201104', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_REPORT_SALES_LIST_20201104
GO
...
/*  
 EXEC SP_REPORT_SALES_LIST_20201104    'LIST'  , ' SA ,  SB , ST , SS , B ,C , H , U , D , Q , P , SG , X , XB , G , SD , '  , ''  ,  0 , '2020-08-24'  , '2020-09-24'  ,  0 ,  0 ,  0 ,  -1 ,  0 ,  0 , ''  , 'N'  , 'N'  , ''  
 EXEC SP_REPORT_SALES_LIST_20201104    'LIST_EXCEL'  , ' SA ,  SB , ST , SS , B ,C , H , U , D , Q , P , SG , X , XB , G , SD , '  , ''  ,  0 , '2020-08-24'  , '2020-09-24'  ,  0 ,  0 ,  0 ,  -1 ,  0 ,  0 , ''  , 'N'  , 'N'  , ''  
*/  
CREATE PROCEDURE [dbo].[SP_REPORT_SALES_LIST_20201104]  
...
GROUP BY CM.Coupon_Type_Code,COC.ORDER_SEQ  
 ...
 FROM    Custom_Order AS CO  
  ...
  LEFT OUTER JOIN S2_UserInfo_TheCard AS UI ON UI.uid = CO.member_id /* 20200714 김성동 본부장님 긴급 추가 요청 */  
...
WHERE order_type = 'U' AND status_seq > 0  
    ...
    (  
	 ...
	 group by ORDER_SEQ
   	...
   	LEFT JOIN
	(
		SELECT member_id, count(*) cnt FROM CUSTOM_ETC_ORDER WHERE ISNULL(member_id,'') <> '' and order_type = '3' GROUP BY member_id
	) AS CCG ON T.member_id = CCG.member_id
   ORDER BY   
                    T.Order_Date DESC  
  END  
