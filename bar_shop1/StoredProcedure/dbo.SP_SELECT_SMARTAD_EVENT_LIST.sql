IF OBJECT_ID (N'dbo.SP_SELECT_SMARTAD_EVENT_LIST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_SELECT_SMARTAD_EVENT_LIST
GO
...
CREATE PROCEDURE [dbo].[SP_SELECT_SMARTAD_EVENT_LIST]    
 ...
 FROM (    
    ...
    AND AD_SEQ IN (124)  
       AND  (    
         ...
         SE.USER_NAME LIKE '%' + @P_SEARCH_VALUE + '%' OR     
         REPLACE(SE.USER_HPHONE,'-','') = @P_SEARCH_VALUE     
         )    
       AND     
         (     
           ISNULL(@P_EVENT_SEQ,'') = '' OR SE.STATUS_CODE IN ( SELECT * FROM [dbo].[ufn_SplitTable] (@P_EVENT_SEQ, '|') )    
         )    
