IF OBJECT_ID (N'dbo.up_select_product_list_stylecolor_search', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_product_list_stylecolor_search
GO
...
*/
-- =============================================
CREATE Procedure [dbo].[up_select_product_list_stylecolor_search]
...
BEGIN
	...
	set @sql = @sql + '	join s2_cardimage AS E	with(nolock) on A.card_seq=E.Card_Seq '
	...
	set @sql = @sql + ' A.Company_Seq='+convert(varchar(10),@company_seq)+' and D.MinCount='+convert(varchar(10), @ordernum)+' and E.CardImage_WSize=''210'' and E.CardImage_HSize=''210'' and E.cardimage_div=''E'' and '
	set @sql = @sql + '	A.IsDisplay=''1'' and E.Company_Seq='+convert(varchar(10),@company_seq)+' '

	exec (@sql)

	set @sql2 =  'select top '+CONVERT(VARCHAR(50),@pagesize)+'  B.card_name, B.card_code, B.cardbrand, B.cardset_price, B.card_seq, B.RegDate, '
	set @sql2 = @sql2 + '	convert(integer, d.discount_rate) AS discount_rate , E.cardimage_filename, A.CardDiscount_Seq, A.company_seq, isSample, isNew, isBest, isSSPre,  '
	...
	set @sql2 = @sql2 + '	join s2_cardimage AS E	with(nolock) on A.card_seq=E.Card_Seq '
	...
	set @sql2 = @sql2 + ' and E.CardImage_WSize=''210'' and E.CardImage_HSize=''210'' and E.cardimage_div=''E'' and   '
	set @sql2 = @sql2 + '	A.IsDisplay=''1'' and E.Company_Seq='+convert(varchar(10),@company_seq)+' '
