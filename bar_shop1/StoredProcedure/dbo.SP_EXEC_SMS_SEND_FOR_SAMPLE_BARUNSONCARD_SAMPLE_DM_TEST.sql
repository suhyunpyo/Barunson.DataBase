IF OBJECT_ID (N'dbo.SP_EXEC_SMS_SEND_FOR_SAMPLE_BARUNSONCARD_SAMPLE_DM_TEST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_EXEC_SMS_SEND_FOR_SAMPLE_BARUNSONCARD_SAMPLE_DM_TEST
GO
...
*/  
CREATE PROCEDURE [dbo].[SP_EXEC_SMS_SEND_FOR_SAMPLE_BARUNSONCARD_SAMPLE_DM_TEST]  
...
BEGIN   
    ...
    */  
    ...
    FOR    
        ...
        /* 한번 발송된 고객 제외(회원, 비회원) */  
        AND     CSO.MEMBER_ID       NOT IN ( SELECT UID FROM COUPON_ISSUE WHERE COMPANY_SEQ = 5001 AND COUPON_DETAIL_SEQ = 47182 )  
        --AND     CSO.MEMBER_EMAIL    NOT IN ( SELECT UID FROM COUPON_ISSUE WHERE COMPANY_SEQ = 5001 AND COUPON_DETAIL_SEQ = 47182 )  
