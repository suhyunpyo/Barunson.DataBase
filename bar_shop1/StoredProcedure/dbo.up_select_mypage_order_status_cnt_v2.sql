IF OBJECT_ID (N'dbo.up_select_mypage_order_status_cnt_v2', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_mypage_order_status_cnt_v2
GO
...
-- 2018.12.05 더카드 비회원 추가로 인한 변수 추가 
-- TEST : up_select_mypage_order_status_cnt 5007, 'palaoh'

-- =============================================
create PROCEDURE [dbo].[up_select_mypage_order_status_cnt_v2]
	@isMember		varchar(1),
	@company_seq	int,
	@uid			nvarchar(50)	
		...
		BEGIN
			...
			(	
				...
				(	
					...
					FROM Custom_Sample_Order
					WHERE 1 = 1
					 AND member_email = @uid
					 AND company_seq = @company_seq
					 AND status_seq >= 1	
				) O ON G.order_g_seq = O.order_g_seq
		
				WHERE G.company_seq = @company_seq
				  AND G.order_email = @uid
