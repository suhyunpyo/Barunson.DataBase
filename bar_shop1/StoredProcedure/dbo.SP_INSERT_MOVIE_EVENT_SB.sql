IF OBJECT_ID (N'dbo.SP_INSERT_MOVIE_EVENT_SB', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_INSERT_MOVIE_EVENT_SB
GO
...
CREATE PROCEDURE [dbo].[SP_INSERT_MOVIE_EVENT_SB]
	@ORDER_SEQ       					AS INT,
	@UID								AS VARCHAR(50)
AS
BEGIN
	...
	DECLARE		@COUPON_CODE					AS	VARCHAR(50) = ''
	    ...
	    ,		@COMPANY_SEQ					AS	INT = 5001
        ,       @COUPON_MST_SEQ                 AS  INT = 301
        ,       @ORDER_CNT                      AS  INT
        ,       @SALES_GUBUN                    AS  VARCHAR(2) = 'SB'
