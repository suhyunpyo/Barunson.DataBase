IF OBJECT_ID (N'dbo.SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_DEARDEER', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_DEARDEER
GO
...
CREATE PROCEDURE [dbo].[SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_DEARDEER]
...
BEGIN
	...
	SET @SEARCH_DATE_END_OF_DAY = CAST(@P_SEARCH_END_DATE + ' 23:59:59' AS DATETIME)
		...
		SELECT	1 AS STATUS
			...
			,	CASE WHEN ISNULL(CSO.MEMBER_ID, '') <> '' AND VUI.WEDD_YEAR <> '' AND VUI.WEDD_MONTH <> '' AND VUI.WEDD_DAY <> '' THEN ISNULL(VUI.WEDD_YEAR + '-' + VUI.WEDD_MONTH + '-' + VUI.WEDD_DAY, '') 
						...
						THEN ISNULL	(
										STUFF	(
													(
														...
														AND     STATUS_SEQ >= 1
														AND     (MEMBER_ID = CSO.MEMBER_ID OR (MEMBER_HPHONE = CSO.MEMBER_HPHONE))
			...
			,   CASE 
						...
						THEN ISNULL	(
										( 
                                            ...
                                            AND     (MEMBER_ID = CSO.MEMBER_ID OR order_hphone = CSO.MEMBER_HPHONE)
                                            AND     ISNULL(MEMBER_ID, '') <> ''
                                            AND     ISNULL(CSO.MEMBER_ID, '') <> ''
										)
									, 0) 
