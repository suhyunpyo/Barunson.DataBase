IF OBJECT_ID (N'dbo.SP_REPORT_SALES_LIST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_REPORT_SALES_LIST
GO
...
*/  
CREATE PROCEDURE [dbo].[SP_REPORT_SALES_LIST]  
...
CREATE TABLE #custom_order_year (
	...
	[NonUserYnSample] [char](1) NULL,
	[NonUserYnOrder] [char](1) NULL,
	[card_seq] [int] NULL,
...
BEGIN
 ...
 ELSE IF @P_List_Type = 'List_Excel'  
  ...
  GROUP BY CWO.Member_ID
    ...
    , MAX(NonUserYn) AS NonUserYnSample
...
and ((isnull(member_id, '') = '' and sales_gubun in ('SB', 'SS', 'B')) or sales_gubun = 'SD')
   ...
   SELECT DISTINCT  
    ...
    , CSO.NonUserYnSample AS R_NonUserYnSample
   ...
   FROM #T AS T WITH(NOLOCK) 
	  ...
	  --  LEFT OUTER JOIN  
		...
		select 
			...
			,case when Member_ID = '비회원' THEN (CASE WHEN SR.MEMBER_HPHONE is null THEN '' ELSE SR.NonUserYnSample END) ELSE R_NonUserYnSample END AS NonUserYnSample
		...
		from #R AS R with (nolock)
			left join (
				select member_hphone,
					...
					max(NonUserYnSample) AS NonUserYnSample
