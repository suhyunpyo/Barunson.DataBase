IF OBJECT_ID (N'dbo.SP_PP_MAIN_RANKING_LIST_SUPERSALE_20200602', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_PP_MAIN_RANKING_LIST_SUPERSALE_20200602
GO
...
-- =======================================================================================
CREATE PROCEDURE [dbo].[SP_PP_MAIN_RANKING_LIST_SUPERSALE_20200602] 
...
BEGIN
	...
	IF @brand = 'I' OR @brand = 'N' OR @brand = 'H' OR @brand = 'L' --I : 인기상품 , N : 신상품
		BEGIN
		IF @brand = 'H'
		BEGIN	
			SET @brand = 'N'
		...
		SET @strQuery = @strQuery + ' 	,title , link_url_img, link_url,  link_target, view_div, jehu_view_div, md_content ,aa ,p_cnt, supersale_Card, coupon_disrate, CardKind_Seq												     '+char(13) + char(10)
		...
		SET @strQuery = @strQuery + '			   ,cnt p_cnt, supersale_Card, coupon_disrate, CardKind_Seq																						     '+char(13) + char(10)
		SET @strQuery = @strQuery + '		 from (																								     '+char(13) + char(10)
		...
		SET @strQuery = @strQuery + '					replace( convert( VARCHAR, convert(money , (Round(cardset_price * ((100 - D.Discount_Rate) / 100) , 0)) * @IN_ordernum), 1 ), ''.00'', '''' )  card_price , Wishcard, sampleCnt, supersale_card	,coupon_disrate, CardKind_Seq     '+char(13) + char(10)

		SET @strQuery = @strQuery + '				from S4_Ranking_Sort_Table AS A left outer join (select distinct B.Card_Seq, C.company_seq, isbest, isnew, isextra, isextra2, isjumun,											     '+char(13) + char(10)
		...
		SET @strQuery = @strQuery + '						,ISNULL((SELECT top 1  ''Y'' FROM S4_RANKING_SORT_TABLE WHERE  RK_CARD_CODE = B.CARD_SEQ AND RK_ST_SEQ = 221) , ''N'') SUPERSALE_CARD	'+char(13) + char(10)
		SET @strQuery = @strQuery + '						,ISNULL((SELECT top 1 discount_value from coupon_mst where coupon_name = ''SUPER SALE_''+card_code ),30) coupon_disrate, K.CardKind_Seq	'+char(13) + char(10)
		...
		IF @company_seq = 5003
		BEGIN
		SET @strQuery = @strQuery + '						join s2_cardkind K with(nolock)  on B.Card_Seq=K.Card_Seq							     '+char(13) + char(10)
		SET @strQuery = @strQuery + '						join s2_cardkindinfo L with(nolock)  on K.cardkind_seq=L.cardkind_seq							     '+char(13) + char(10)
		END 
		...
		IF @company_seq = 5003
		BEGIN
		SET @strQuery = @strQuery + '						and K.CardKind_Seq in (1,2)													     '+char(13) + char(10)
		END 
		...
		SET @strQuery = @strQuery + '			,title , link_url_img,link_url,  link_target, view_div, jehu_view_div, md_content ,'''' aa										     '+char(13) + char(10)
		SET @strQuery = @strQuery + '			,0 p_cnt, ''N'' supersale_Card, 0 coupon_disrate, 1 CardKind_Seq																						     '+char(13) + char(10)
		SET @strQuery = @strQuery + '		 from (																								     '+char(13) + char(10)
		...
		IF @ORDERPRICE = 'H'
		BEGIN 
			SET @strQuery = @strQuery + ' order by card_price DESC'
		END 
		ELSE IF  @ORDERPRICE = 'L'
		BEGIN 
			SET @strQuery = @strQuery + ' order by card_price ASC'
		END
		ELSE 
		BEGIN
			IF @V_PAGE > 0 --페이징
		...
		SET @strQuery = @strQuery + '	card_seq,isbest, isnew, isextra,  card_code, CARD_NAME as card_name, cardset_price,card_content,	discount_rate, cardimage_filename,	issample, '+char(13) + char(10)
		SET @strQuery = @strQuery + '	cardsale_price card_sale_price, card_text_premier, regdate,isfsc,  '+char(13) + char(10)
		SET @strQuery = @strQuery + '	replace( convert( varchar, convert(money , (round(cardset_price * ((100 - d.discount_rate) / 100) , 0)) * @IN_ordernum), 1 ), ''.00'', '''' )  card_price , wishcard, sampleCnt,''M'' gb'+char(13) + char(10)
		...
		IF @V_PAGE > 0 --페이징
		BEGIN
			SET @strQuery = @strQuery + 'WHERE rm > ( @IN_V_PAGE -1 ) * @IN_V_PAGESIZE  and rm <=  ( @IN_V_PAGE  * @IN_V_PAGESIZE )'
		END 
	...
	ELSE	--H : 고가상품 , L : 저가상품
		...
		SET @strQuery = @strQuery + ' 	,card_text_premier	,regdate	,isFSC ,card_price,wishcard, sampleCnt' +char(13) + char(10)
		SET @strQuery = @strQuery + ' 	,title , link_url_img, link_url,  link_target, view_div, jehu_view_div, md_content ,aa, supersale_Card ' +char(13) + char(10)
		SET @strQuery = @strQuery + ' FROM (' +char(13) + char(10)
		SET @strQuery = @strQuery + ' 		SELECT  ROW_NUMBER()OVER(ORDER BY RE_RM_NO) NUM,count(*)over() cnt,*' +char(13) + char(10)
		SET @strQuery = @strQuery + ' 		FROM (' +char(13) + char(10)
		...
		SET @strQuery = @strQuery + '       ,card_seq ,isbest	,isnew	,isextra	,card_code	,card_name	,cardset_price	,discount_rate	,cardimage_filename	,issample	,card_sale_price' +char(13) + char(10)
		SET @strQuery = @strQuery + '	   ,card_text_premier	,regdate	,isFSC ,card_price, Wishcard,sampleCnt' +char(13) + char(10)
		SET @strQuery = @strQuery + '	   ,'''' title , '''' link_url_img, ''''link_url,  '''' link_target, '''' view_div, '''' jehu_view_div, '''' reg_date ,'''' md_content ,'''' aa, supersale_Card ' +char(13) + char(10)
		SET @strQuery = @strQuery + ' from (' +char(13) + char(10)
		...
		SET @strQuery = @strQuery + '				,card_text_premier	,regdate	,isFSC  ,Wishcard, sampleCnt ,replace( convert( VARCHAR, convert(money , (Round(cardset_price * ((100 - discount_rate) / 100) , 0)) * @IN_ordernum), 1 ), ''.00'', '''' )  card_price, card_sale_price ' +char(13) + char(10)
		SET @strQuery = @strQuery + '		from (' +char(13) + char(10)
		...
		SET @strQuery = @strQuery + '		,card_seq ,'''' isbest ,'''' isnew	, '''' isextra ,'''' card_code	,'''' card_name,0 cardset_price,0 discount_rate	,'''' cardimage_filename	,'''' issample ,0 card_sale_price' +char(13) + char(10)
		SET @strQuery = @strQuery + '		,'''' card_text_premier,'''' regdate	,'''' isFSC ,'''' card_price ,''N'' Wishcard, sampleCnt' +char(13) + char(10)
