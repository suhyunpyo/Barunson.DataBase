IF OBJECT_ID (N'dbo.SP_INSERT_INTEGRATION_MEMBER_SIGN_UP_NEW', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_INSERT_INTEGRATION_MEMBER_SIGN_UP_NEW
GO
...
GO
CREATE PROC [dbo].[SP_INSERT_INTEGRATION_MEMBER_SIGN_UP_NEW] 
...
AS  
 SET NOCOUNT ON BEGIN
	...
	-- CUCKOO
	DECLARE @CUCKOOSSHIP_REG_DATE AS VARCHAR(19) = CONVERT(VARCHAR(19), GETDATE(), 120)
	SET @CUCKOOSSHIP_REG_DATE = CONVERT(VARCHAR(19), GETDATE(), 120)
	IF EXISTS(SELECT TOP 1 * FROM VW_USER_INFO WHERE DUPINFO = @AUTHCODE)
	BEGIN
		SET @CUCKOOSSHIP_REG_DATE = (SELECT CONVERT(VARCHAR(19), MIN(@CUCKOOSSHIP_REG_DATE), 120) AS CUCKOOSSHIP_REG_DATE FROM VW_USER_INFO WHERE DUPINFO = @AUTHCODE)
	END
	
	-- 까사미아
	DECLARE @CASAMIASHIP_REG_DATE AS VARCHAR(19) = CONVERT(VARCHAR(19), GETDATE(), 120)
	SET @CASAMIASHIP_REG_DATE = CONVERT(VARCHAR(19), GETDATE(), 120)
	IF EXISTS(SELECT TOP 1 * FROM VW_USER_INFO WHERE DupInfo = @AUTHCODE)
	BEGIN
		SET @CASAMIASHIP_REG_DATE = (SELECT CONVERT(VARCHAR(19), MIN(@CASAMIASHIP_REG_DATE), 120) AS CASAMIASHIP_REG_DATE FROM VW_USER_INFO WHERE DupInfo = @AUTHCODE)
	END

	-- KT
	DECLARE @KTMEMBERSHIP_REG_DATE AS VARCHAR(19) = CONVERT(VARCHAR(19), GETDATE(), 120)
	SET @KTMEMBERSHIP_REG_DATE = CONVERT(VARCHAR(19), GETDATE(), 120)
	IF EXISTS(SELECT TOP 1 * FROM VW_USER_INFO WHERE DupInfo = @AUTHCODE)
	BEGIN
		SET @KTMEMBERSHIP_REG_DATE = (SELECT CONVERT(VARCHAR(19), MIN(@KTMEMBERSHIP_REG_DATE), 120) AS KTMEMBERSHIP_REG_DATE FROM VW_USER_INFO WHERE DupInfo = @AUTHCODE)
	END
	
	
	-- 현대백화점
	DECLARE @HYUNDAIMEMBERSHIP_REG_DATE AS VARCHAR(19) = CONVERT(VARCHAR(19), GETDATE(), 120)
	SET @HYUNDAIMEMBERSHIP_REG_DATE = CONVERT(VARCHAR(19), GETDATE(), 120)
	IF EXISTS(SELECT TOP 1 * FROM VW_USER_INFO WHERE DupInfo = @AUTHCODE)
	BEGIN
		SET @HYUNDAIMEMBERSHIP_REG_DATE = (SELECT CONVERT(VARCHAR(19), MIN(@HYUNDAIMEMBERSHIP_REG_DATE), 120) AS HYUNDAIMEMBERSHIP_REG_DATE FROM VW_USER_INFO WHERE DupInfo = @AUTHCODE)
	END
    ...
    IF (@CUCKOOSSHIP_REG_DATE) IS NULL
    BEGIN
	    SET @CUCKOOSSHIP_REG_DATE = CONVERT(VARCHAR(19), GETDATE(), 120)
	END
	
	IF (@CASAMIASHIP_REG_DATE) IS NULL
	BEGIN
		SET @CASAMIASHIP_REG_DATE = CONVERT(VARCHAR(19), GETDATE(), 120)
	END

	IF (@KTMEMBERSHIP_REG_DATE) IS NULL
	BEGIN
		SET @KTMEMBERSHIP_REG_DATE = CONVERT(VARCHAR(19), GETDATE(), 120)
	END

	IF (@HYUNDAIMEMBERSHIP_REG_DATE) IS NULL
	BEGIN
		SET @HYUNDAIMEMBERSHIP_REG_DATE = CONVERT(VARCHAR(19), GETDATE(), 120)
	END
    ...
    IF NOT EXISTS(SELECT TOP 1 * FROM S2_USERINFO_BHANDS WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SA' , 'B', 'C' )) BEGIN
        ...
        SELECT  
            ...
            , 	CASE WHEN @CHK_CUCKOOSMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
            ,	CASE WHEN @CHK_CUCKOOSMEMBERSHIP = 'Y' THEN @CUCKOOSSHIP_REG_DATE ELSE NULL END
            ,	CASE WHEN @CHK_CASAMIAMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
            ,	CASE WHEN @CHK_CASAMIAMEMBERSHIP = 'Y' THEN @CASAMIASHIP_REG_DATE ELSE NULL END
            ,	CASE WHEN @CHK_KTMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
            ,	CASE WHEN @CHK_KTMEMBERSHIP = 'Y' THEN @KTMEMBERSHIP_REG_DATE ELSE NULL END
            , 	@WEDD_NAME
            ,	@SMEMBERSHIP_PERIOD
			,	CASE WHEN @CHK_HYUNDAIMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
            ,	CASE WHEN @CHK_HYUNDAIMEMBERSHIP = 'Y' THEN @HYUNDAIMEMBERSHIP_REG_DATE ELSE NULL END
    ...
    -- 전환가입시 CUCKOO동의 체크 신규 추가 2021-02-02
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO_BHANDS WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SA' , 'B', 'C' ) AND CHK_CUCKOOSMEMBERSHIP IS NULL) AND @CHK_CUCKOOSMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO_BHANDS
	    	SET 
	    		CHK_CUCKOOSMEMBERSHIP	= CASE WHEN @CHK_CUCKOOSMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
            	, CUCKOOSSHIP_REG_DATE	= CASE WHEN @CHK_CUCKOOSMEMBERSHIP = 'Y' THEN @CUCKOOSSHIP_REG_DATE ELSE NULL END
            WHERE
            	DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SA' , 'B', 'C' )
    END 
    
    -- 전환가입시 까사미아 동의 체크 신규 추가 2021-12-13
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO_BHANDS WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SA' , 'B', 'C' ) AND CHK_CASAMIAMEMBERSHIP IS NULL) AND @CHK_CASAMIAMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO_BHANDS
	    	SET 
	    		CHK_CASAMIAMEMBERSHIP	= CASE WHEN @CHK_CASAMIAMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
            	, CASAMIASHIP_REG_DATE	= CASE WHEN @CHK_CASAMIAMEMBERSHIP = 'Y' THEN @CASAMIASHIP_REG_DATE ELSE NULL END
            WHERE
            	DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SA' , 'B', 'C' )
    END 

    -- 전환가입시 KT 동의 체크 2022-03-29
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO_BHANDS WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SA' , 'B', 'C' ) AND CHK_KTMEMBERSHIP IS NULL) AND @CHK_KTMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO_BHANDS
	    	SET 
	    		CHK_KTMEMBERSHIP	= CASE WHEN @CHK_KTMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
            	, KTMEMBERSHIP_REG_DATE	= CASE WHEN @CHK_KTMEMBERSHIP = 'Y' THEN @KTMEMBERSHIP_REG_DATE ELSE NULL END
            WHERE
            	DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SA' , 'B', 'C' )
    END 
	...
	-- 전환가입시 현대백화점 동의 체크 신규 추가 2022-04-27
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO_BHANDS WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SA' , 'B', 'C' ) AND CHK_HYUNDAIMEMBERSHIP IS NULL) AND @CHK_HYUNDAIMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO_BHANDS
	    	SET 
	    		CHK_HYUNDAIMEMBERSHIP	= CASE WHEN @CHK_HYUNDAIMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
            	, HYUNDAIMEMBERSHIP_REG_DATE	= CASE WHEN @CHK_HYUNDAIMEMBERSHIP = 'Y' THEN @HYUNDAIMEMBERSHIP_REG_DATE ELSE NULL END
            WHERE
            	DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SA' , 'B', 'C' )
    END 
...
,   @ADDRESS_DETAIL
    ...
    -- 전환가입시 CUCKOO동의 체크 신규 추가 2021-08-23
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO_THECARD WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'ST' ) AND CHK_CUCKOOSMEMBERSHIP IS NULL) AND @CHK_CUCKOOSMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO_THECARD
	    	SET 
	    		CHK_CUCKOOSMEMBERSHIP	= CASE WHEN @CHK_CUCKOOSMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
    ...
    -- 전환가입시 까사미아동의 체크 신규 추가 2021-08-23
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO_THECARD WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'ST' ) AND CHK_CASAMIAMEMBERSHIP IS NULL) AND @CHK_CASAMIAMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO_THECARD
	    	SET 
	    		CHK_CASAMIAMEMBERSHIP	= CASE WHEN @CHK_CASAMIAMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
    ...
    -- 전환가입시 KT동의 체크 신규 추가 2022-03-29
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO_THECARD WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'ST' ) AND CHK_KTMEMBERSHIP IS NULL) AND @CHK_KTMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO_THECARD
	    	SET 
	    		CHK_KTMEMBERSHIP	= CASE WHEN @CHK_KTMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
	...
	END
	  ...
	  -- 전환가입시 현대백화점 동의 체크 신규 추가 2022-04-27
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO_THECARD WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'ST' ) AND CHK_HYUNDAIMEMBERSHIP IS NULL) AND @CHK_HYUNDAIMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO_THECARD
	    	SET 
	    		CHK_HYUNDAIMEMBERSHIP	= CASE WHEN @CHK_HYUNDAIMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
  ...
  -- 전환가입시 LG동의 체크 신규 추가 2021-02-02
    ...
    -- 전환가입시 CUCKOO동의 체크 신규 추가 2021-08-23
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SB' ) AND CHK_CUCKOOSMEMBERSHIP IS NULL) AND @CHK_CUCKOOSMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO
	    	SET 
	    		CHK_CUCKOOSMEMBERSHIP	= CASE WHEN @CHK_CUCKOOSMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
    ...
    -- 전환가입시 까사미아 동의 체크 신규 추가 2021-08-23
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SB' ) AND CHK_CASAMIAMEMBERSHIP IS NULL) AND @CHK_CASAMIAMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO
	    	SET 
	    		CHK_CASAMIAMEMBERSHIP	= CASE WHEN @CHK_CASAMIAMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
    ...
    END
        ...
        -- 전환가입시 KT 동의 체크 신규 추가 2022-03-29
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SB' ) AND CHK_KTMEMBERSHIP IS NULL) AND @CHK_KTMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO
	    	SET 
	    		CHK_KTMEMBERSHIP	= CASE WHEN @CHK_KTMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
	...
	END
      ...
      -- 전환가입시 현대백화점 동의 체크 신규 추가 2022-04-27
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SB' ) AND CHK_HYUNDAIMEMBERSHIP IS NULL) AND @CHK_HYUNDAIMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO
	    	SET 
	    		CHK_HYUNDAIMEMBERSHIP	= CASE WHEN @CHK_HYUNDAIMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
  ...
  ,   @WEDD_MONTH
    ...
    -- 전환가입시 CUCKOO동의 체크 신규 추가 2021-08-23
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SS' , 'H' ) AND CHK_CUCKOOSMEMBERSHIP IS NULL) AND @CHK_CUCKOOSMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO
	    	SET 
	    		CHK_CUCKOOSMEMBERSHIP	= CASE WHEN @CHK_CUCKOOSMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
    ...
    -- 전환가입시 까사미아동의 체크 신규 추가 2021-08-23
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SS' , 'H' ) AND CHK_CASAMIAMEMBERSHIP IS NULL) AND @CHK_CASAMIAMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO
	    	SET 
	    		CHK_CASAMIAMEMBERSHIP	= CASE WHEN @CHK_CASAMIAMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
    ...
    -- 전환가입시 KT동의 체크 신규 추가 2022-03-29
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SS' , 'H' ) AND CHK_KTMEMBERSHIP IS NULL) AND @CHK_KTMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO
	    	SET 
	    		CHK_KTMEMBERSHIP	= CASE WHEN @CHK_KTMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
	...
	-- 전환가입시 현대백화점 동의 체크 신규 추가 2022-04-27
    IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ( 'SS' , 'H' ) AND CHK_HYUNDAIMEMBERSHIP IS NULL) AND @CHK_HYUNDAIMEMBERSHIP = 'Y'
    BEGIN
	    UPDATE S2_USERINFO
	    		SET 
	    		CHK_HYUNDAIMEMBERSHIP	= CASE WHEN @CHK_HYUNDAIMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END
	...
	IF NOT EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV = 'BM') BEGIN 
		...
		INSERT INTO S2_USERINFO ( DUPINFO, CONNINFO, AUTHTYPE, UID, PWD, UNAME, UMAIL, SITE_DIV, JUMIN, BIRTH, 
								  ...
								  ILOOMMEMBERSHIP_REG_DATE, CHK_LGMEMBERSHIP, LGMEMBERSHIP_REG_DATE, CHK_CUCKOOSMEMBERSHIP, CUCKOOSSHIP_REG_DATE
								  , CHK_CASAMIAMEMBERSHIP, CASAMIASHIP_REG_DATE, CHK_KTMEMBERSHIP, KTMEMBERSHIP_REG_DATE, WEDD_NAME, SMEMBERSHIP_PERIOD,
								     CHK_HYUNDAIMEMBERSHIP
								,	HYUNDAIMEMBERSHIP_REG_DATE
    ...
    IF EXISTS(
				...
				END 
								...
								-- 전환가입시 LG동의 체크 신규 추가 2021-02-02
				IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ('BM') AND CHK_CUCKOOSMEMBERSHIP IS NULL) AND @CHK_CUCKOOSMEMBERSHIP = 'Y' BEGIN
					UPDATE S2_USERINFO
					SET CHK_CUCKOOSMEMBERSHIP = CASE WHEN @CHK_CUCKOOSMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END, 
						CUCKOOSSHIP_REG_DATE = CASE WHEN @CHK_CUCKOOSMEMBERSHIP = 'Y' THEN @CUCKOOSSHIP_REG_DATE ELSE NULL END
					WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ('BM')
				END 
				
				-- 전환가입시 까사미아동의 체크 신규 추가 2021-02-02
				IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ('BM') AND CHK_CASAMIAMEMBERSHIP IS NULL) AND @CHK_CASAMIAMEMBERSHIP = 'Y' BEGIN
					UPDATE S2_USERINFO
					SET CHK_CASAMIAMEMBERSHIP = CASE WHEN @CHK_CASAMIAMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END, 
						CASAMIASHIP_REG_DATE = CASE WHEN @CHK_CASAMIAMEMBERSHIP = 'Y' THEN @CASAMIASHIP_REG_DATE ELSE NULL END
					WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ('BM')
				END 		

				-- 전환가입시 KT 동의 체크 신규 추가 2022-03-29
				IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ('BM') AND CHK_KTMEMBERSHIP IS NULL) AND @CHK_KTMEMBERSHIP = 'Y' BEGIN
					UPDATE S2_USERINFO
					SET CHK_KTMEMBERSHIP = CASE WHEN @CHK_KTMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END, 
						KTMEMBERSHIP_REG_DATE = CASE WHEN @CHK_KTMEMBERSHIP = 'Y' THEN @KTMEMBERSHIP_REG_DATE ELSE NULL END
					WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ('BM')
				END 			
				...
				-- 전환가입시 현대백화점 동의 체크 신규 추가 2022-04-27
				IF EXISTS(SELECT TOP 1 * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ('BM') AND CHK_HYUNDAIMEMBERSHIP IS NULL) AND @CHK_HYUNDAIMEMBERSHIP = 'Y' BEGIN
					UPDATE S2_USERINFO
					SET CHK_HYUNDAIMEMBERSHIP	= CASE WHEN @CHK_HYUNDAIMEMBERSHIP = 'Y' THEN 'Y' ELSE 'N' END, 
					HYUNDAIMEMBERSHIP_REG_DATE	= CASE WHEN @CHK_HYUNDAIMEMBERSHIP = 'Y' THEN @HYUNDAIMEMBERSHIP_REG_DATE ELSE NULL END
					WHERE DUPINFO = @AUTHCODE AND SITE_DIV IN ('BM')
				END 		

				--IF EXISTS(SELECT * FROM S2_USERINFO WHERE DUPINFO = @AUTHCODE AND SITE_DIV = 'BM' AND MKT_CHK_FLAG = 'N' ) begin 
				--	SET @MKT_CHK_FLAG = 'N'
				--end else begin SET @MKT_CHK_FLAG = 'Y' end 
