IF OBJECT_ID (N'dbo.SP_EXEC_DELIVERY_RECEIPT_FOR_CJ_NEW', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_EXEC_DELIVERY_RECEIPT_FOR_CJ_NEW
GO
...
*/
CREATE PROCEDURE [dbo].[SP_EXEC_DELIVERY_RECEIPT_FOR_CJ_NEW]
...
BEGIN TRY
    ...
    DECLARE DELIVERY_CURSOR CURSOR LOCAL FOR
	 ...
	 SELECT ORDER_SEQ = A.ORDER_SEQ,
			...
			MPCK_KEY = CONVERT(VARCHAR(8), A.SEND_DATE, 112) + '_' + @CUST_ID + '_' + A.DELIVERY_CODE + '_1_' + CAST(A.DELIVERY_SEQ AS VARCHAR(10)) ,
			...
			PROD_NAME = CASE WHEN A.ORDER_TABLE_NAME = 'CUSTOM_ORDER' THEN '청첩장'
							 ...
							 ELSE '기타 상품'
						END,
			SEND_DATE = CONVERT(VARCHAR(8), A.SEND_DATE, 112) ,
			...
			BUSEONAME = CASE WHEN B.ERP_PARTCODE = '110' THEN '바른손카드' 
						 ...
						 ELSE '' END
	FROM    VW_DELIVERY_MST A 
		INNER JOIN COMPANY B
			on A.COMPANY_SEQ = B.COMPANY_SEQ
		Left JOIN CJ_DELCODE C
			ON a.DELIVERY_CODE = c.CODE
	WHERE   1 = 1
