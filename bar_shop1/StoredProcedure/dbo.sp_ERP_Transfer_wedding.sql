IF OBJECT_ID (N'dbo.sp_ERP_Transfer_wedding', N'P') IS NOT NULL DROP PROCEDURE dbo.sp_ERP_Transfer_wedding
GO
...
-- ########################################################################################################## 
--EXEC sp_ERP_Transfer_wedding '20100218','20100218'


CREATE                       procedure [dbo].[sp_ERP_Transfer_wedding]
 @SDate as char(8),
 @EDate as char(8)
as
...
SELECT 
	...
	h_sysCode	= Case 
				...
				When b.item_Type ='R'                                           	Then '270'    --부속품R 이상민-20100304 
	...
	h_comcode	=   Case  
				...
				When a.company_seq =  2556				Then     '1400113'   --현대카드
	...
	--              	--팔방에프엔비          1300748
			...
			--현대41컨벤션웨딩    1495184
			--마이e웨딩  2400121
              h_taxType	= CASE WHEN ISNULL(RTRIM(LTRIM(c.erp_code)), '') <> '' AND O.TaxCodeAr IS NOT NULL  THEN O.TaxCodeAr ELSE '22' END,		-- ERP 거래처등록에 셋팅되어 있는 과세구분(매출) 데이터 가져오도록 수정. 20090807_이상민
			-- Case When   c.erp_code in ('1300748','1495193','1495190','1495184','2400121')  Then '10' 
			--	Else '22' End
	...
	--**************************************************************************************************************
	FeeAmnt		=  dbo.getPGFee_New (a.pg_shopid, a.settle_method, a.settle_price),
	...
	PGCheck	=  Case
				When a.company_seq in (532,553,587,1186,1272,2590,2556)  Then 'Y'
...
FROM 
	custom_order a JOIN  (
				...
				FROM 
					(
					--기본 아이템 정보 
					SELECT  a.order_seq, b.card_seq, c.Card_ERPCode, b.item_type,b.item_count as item_count ,  IsNUll(b.item_sale_price,0) as item_price
					...
					and  a.status_seq = 15 
					and pay_type <> '4' 
					and (a.company_seq in (532,553,587,1186,2107,2590,2556)  --삼성카드 임직원, 삼성카드회원,결사모,결혼도움방, 마이e웨딩
				...
				UNION   
					...
					and  status_seq = 15 
					and pay_type <> '4' 
					and (company_seq in (532,553,587,1186,2107,2590,2556)  
	...
	LEFT JOIN [erpdb.bhandscard.com].XERP.DBO.CsMaster O ON c.erp_code = O.CsCode AND O.ComCode = 'BK01'		-- ERP 거래처등록에 셋팅되어 있는 과세구분(매출) 데이터 가져오도록 수정. 20090807_이상민
WHERE 
Convert(char(8),a.src_send_date,112) between @SDate and @EDate
and  a.status_seq = 15 
and pay_type <> '4' 
and 
(
	c.company_seq in (532,553,587,1186,2107,2590,2556)  --삼성카드 임직원, 삼성카드회원,결사모,결혼도움방, 마이e웨딩, 현대카드
	or
...
SELECT 
	...
	b_memo		= null,
	reg_date		= getdate(),
	FeeAmnt		=  dbo.getPGFee_New (a.pg_mertid, a.settle_method, a.settle_price),
...
FROM 
	...
	JOIN company c ON a.company_seq = c.company_seq
	JOIN #CardMaster d ON b.card_seq = d.card_seq
	LEFT JOIN  OPENQUERY([erpdb.bhandscard.com], 'SELECT  IsNull(stdPurprice,0) as stdPurprice, IsNull(c_sobi,0) as c_sobi,itemcode,itemgroup FROM XERP.DBO.itemSiteMaster ') e ON d.Card_ERPCode = e.itemCode
	LEFT JOIN [erpdb.bhandscard.com].XERP.DBO.CsMaster O ON c.erp_code = O.CsCode AND O.ComCode = 'BK01'		-- ERP 거래처등록에 셋팅되어 있는 과세구분(매출) 데이터 가져오도록 수정. 20090807_이상민
...
and 
(
	c.company_seq in (532,553,587,1186,2107,2590,2556)  --삼성카드 임직원, 삼성카드회원,결사모,결혼도움방, 마이e웨딩
	or
	a.sales_gubun = 'O' and c.jaehu_kind ='C'  and c.company_seq <> 15 -- 일단 웨딩사이트만 올림, 후불결제(ecSupport)는 추후에..
...
SELECT 
	...
	--              	--팔방에프엔비          1300748
			...
			--현대41컨벤션웨딩    1495184
			--마이e웨딩  2400121
              h_taxType	=  CASE WHEN ISNULL(RTRIM(LTRIM(c.erp_code)), '') <> '' AND O.TaxCodeAr IS NOT NULL  THEN O.TaxCodeAr ELSE '22' END,
	...
	b_storeCode	= 'MF03',
	b_date		= Convert(char(8),a.delivery_date,112),
	b_goodCode	= d.Card_ERPCode, --b.item_type
	b_goodUnit	= 'EA',
