IF OBJECT_ID (N'dbo.SP_INSERT_FREE_GIFT_FOR_VIRTUALBANK_SETTLE_MIST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_INSERT_FREE_GIFT_FOR_VIRTUALBANK_SETTLE_MIST
GO
...
CREATE PROCEDURE [dbo].[SP_INSERT_FREE_GIFT_FOR_VIRTUALBANK_SETTLE_MIST]
...
DECLARE @ISNEW AS VARCHAR(1)

SELECT @ORDER_SEQ = ORDER_SEQ
FROM CUSTOM_ORDER
WHERE PG_TID = @PG_TID AND COMPANY_SEQ = 5006
...
BEGIN
	...
	SELECT  TOP 1
		...
		,   @ISNEW	= (SELECT ISNEW from s2_cardsalessite where company_seq = 5006 and card_seq = CO.card_seq) 
