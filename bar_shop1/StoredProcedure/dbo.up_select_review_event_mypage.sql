IF OBJECT_ID (N'dbo.up_select_review_event_mypage', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_review_event_mypage
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[up_select_review_event_mypage]
	...
	if @txt_card_seq = 0
		begin
			if (@company_seq = '5001' or @company_seq = '5003')	--바른손
				...
				begin
					set @SQL2 = ' select count(ER_Idx) from S4_Event_Review with(nolock) where ER_Type='+CONVERT(VARCHAR(2),@ER_Type)+' and ER_Company_Seq !=5001 and ER_Company_Seq !=5003 and ER_View=0  AND ER_UserID =''' + CONVERT(VARCHAR(50),@UID) + ''' '
					exec (@SQL2)

					set @SQL = 'select top '+ CONVERT(VARCHAR(50),@pagesize) +' ER_Idx, ER_Company_Seq, ER_Order_Seq, ER_Type, ER_Userid, ER_Regdate, ER_Recom_Cnt, isnull(ER_Review_Title, '''') AS ER_Review_Title, isnull(ER_Review_Url, '''') AS ER_Review_Url , '
					set @SQL = @SQL + ' ER_Review_Star, ER_Status, ER_View, ER_Review_Content, ER_card_seq, ER_Card_code, ER_UserName, '
					set @SQL = @SQL + ' ERA_Status, ERA_Coupon_Status, isnull(ERA_Comment,'''') AS ERA_Comment, ERA_Coupon_Code, isnull(ERA_Comment_Cancel,'''') AS ERA_Comment_Cancel, isnull(ER_Review_Url_a, '''') AS ER_Review_Url_a, isnull(ER_Review_Url_b, '''') AS ER_Review_Url_b from S4_Event_Review AS A with(nolock) left outer join  S4_Event_Review_Status AS B with(nolock) on A.ER_Idx = B.ERA_ER_idx '
					set @SQL = @SQL + ' where ER_Type='+CONVERT(VARCHAR(2),@ER_Type)+' AND ER_UserID =''' + CONVERT(VARCHAR(50),@UID) + ''' AND ER_Idx not in (select top '+ CONVERT(VARCHAR(50), @pagesize * (@page - 1)) +' ER_Idx from S4_Event_Review with(nolock) where ER_Company_Seq !=5001 and  ER_Company_Seq !=5003 and ER_View=0  order by '+@sort_desc+' desc  ) '
					set @SQL = @SQL + ' and ER_Company_Seq !=5001  and ER_Company_Seq !=5003  and ER_View=0 AND ER_Type='+CONVERT(VARCHAR(2),@ER_Type)+' AND ER_UserID =''' + CONVERT(VARCHAR(50),@UID) + ''' order by '+@sort_desc+' desc'
					exec (@SQL)
				end
		end
	else
