IF OBJECT_ID (N'dbo.up_select_statics_time_settle', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_statics_time_settle
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[up_select_statics_time_settle]
...
BEGIN
	...
	from
	(
		select  company_seq, left(CONVERT(VARCHAR, settle_date, 8),2) AS STT, count(order_seq) AS cnt
		from    custom_order with(nolock)
		where   convert(varchar(10),settle_date,121) between @sDate and @eDate
		and company_seq in ('5001', '5006', '5007', '5003')
		group by company_seq,
	    
			  left(CONVERT(VARCHAR, settle_date, 8),2)
	) a 
	group by company_seq
