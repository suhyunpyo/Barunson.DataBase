IF OBJECT_ID (N'dbo.sp_select_bhands_member_count', N'P') IS NOT NULL DROP PROCEDURE dbo.sp_select_bhands_member_count
GO
...
create PROCEDURE [dbo].[sp_select_bhands_member_count]
...
-- ///////////////////////// 비핸즈사이트 회원가입 데이터 ////////////////////////////////////////////////////////
--날짜	/요일	/비핸즈전체	/비핸즈웹가입	/비핸즈모바일가입	/멤버십O	/멤버십O가입시	/멤버십O수정시	/멤버십O에프터페이시/멤버십X/멤버십%
-- [sp_select_bhands_member_count] '2014-04-01','2014-05-29'
...
select convert(varchar(10),reg_date,120) 'date', count(*) '비핸즈모바일가입'
from dbo.S2_UserInfo_BHands 
where convert(varchar(10),reg_date,120) >= @sDate and convert(varchar(10),reg_date,120) <= @eDate
and site_div in ('SA','B','C') 
...
from dbo.S2_UserInfo_BHands
where convert(varchar(10),reg_date,120) >= @sDate and convert(varchar(10),reg_date,120) <= @eDate
and site_div in ('SA','B','C')  and chk_smembership ='Y'
...
delete dbo.bhands_day_count

insert into dbo.bhands_day_count
(reg_date,[day],bhands_join_total,bhands_join_web,bhands_join_mobile
,bhands_membership_total,bhands_membership_join,bhands_membership_modify
,bhands_membership_apay,bhands_membership_X,bhands_membership_percent)
...
, a.cnt1 bhands_join_total, b.cnt2 bhands_join_web
, ISNULL(c.cnt3,0) bhands_join_mobile
, d.cnt4 bhands_membership_total, e.cnt5 bhands_membership_join, f.cnt6 bhands_membership_modify
, ISNULL(g.cnt7,0) bhands_membership_apay
, (b.cnt2 - d.cnt4) bhands_membership_X
, (round(d.cnt4/convert(float,(a.cnt1)),2)*100) bhands_membership_percent
from #UsTemp1 a 
...
select * from dbo.bhands_day_count order by reg_date 
