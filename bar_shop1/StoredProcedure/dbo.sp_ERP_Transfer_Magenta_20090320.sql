IF OBJECT_ID (N'dbo.sp_ERP_Transfer_Magenta_20090320', N'P') IS NOT NULL DROP PROCEDURE dbo.sp_ERP_Transfer_Magenta_20090320
GO
...
-- ########################################################################################################## 
--exec sp_ERP_Transfer_Magenta_20090320 '20090128','20090128'
CREATE    procedure [dbo].[sp_ERP_Transfer_Magenta_20090320]
 @SDate as char(8),
 @EDate as char(8)
as
...
FROM 
	...
	LEFT JOIN  OPENQUERY([211.172.242.78], 'SELECT  IsNull(stdPurprice,0) as stdPurprice, IsNull(c_sobi,0) as c_sobi,itemcode,itemgroup FROM XERP.DBO.itemSiteMaster ') e ON d.card_code = e.itemCode
...
--SET 	
---- 	b_offerPrice =  Case 
---- 			When a.h_sumPrice > b.b_sumPrice Then  a.b_offerPrice + (a.h_sumPrice - b.b_sumPrice)
---- 			Else a.b_offerPrice - (b.b_sumPrice - a.h_sumPrice) 
---- 		          End , 
---- 	h_offerPrice = b.b_offerPrice,
---- 	h_superTax = b.b_superTax,
...
--		         End
--FROM 	@erp_salesReport  a 
--JOIN 	(	SELECT h_orderid,sum(b_offerPrice) as b_offerPrice,sum(b_superTax) as b_superTax,sum(b_sumPrice) as b_sumPrice FROM @erp_salesReport
--		GROUP BY h_orderid
--	) b ON a.h_orderid = b.h_orderid
--JOIN 	(
...
--					) B ON A.h_OrderID = B.h_OrderID
--	 ) c ON a.h_orderid = c.h_orderid and a.b_goodCode = c.b_goodCode
--WHERE a.h_sumPrice <> b.b_sumPrice and c.b_Seq = 1 and a.SampleCheck = 'N'
...
FROM @erp_salesReport  a  JOIN (
				...
				--ORDER BY a.h_OrderID, a.b_goodCode, GroupSerNo
			     ...
			     LEFT JOIN OPENQUERY([222.120.91.63], 'SELECT  h_orderid FROM MagentaERP.DBO.erp_salesData ')  c ON a.h_orderid = c.h_orderid
