IF OBJECT_ID (N'dbo.SP_SELECT_OUTSOURCING_ORDER_MST_FOR_LIST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_SELECT_OUTSOURCING_ORDER_MST_FOR_LIST
GO
...
CREATE PROCEDURE [dbo].[SP_SELECT_OUTSOURCING_ORDER_MST_FOR_LIST]
...
BEGIN
    ...
    SELECT  ROW_NUMBER() OVER(ORDER BY A.REG_DATE ASC) AS ROW_NUM
		...
		, A.PRINT_CHASU
		, A.MEMO_EX
		, (select hardcode+',' from HardCodingList where HardID = 'AUTH_OUTSOURCING_DEL' and HardUse = 'Y' for XML PATH('')) AUTH_USER
    ...
    AND     (@P_COMPANY_TYPE_CODE = '999999' or A.COMPANY_TYPE_CODE LIKE (CASE WHEN @P_COMPANY_TYPE_CODE <> '' THEN @P_COMPANY_TYPE_CODE ELSE '%' END))

    AND     (@P_COMPANY_TYPE_CODE <> '999999' or (c.card_code is not null and (A.COMPANY_TYPE_CODE = '107012' OR A.COMPANY_TYPE_CODE = '107014')))
