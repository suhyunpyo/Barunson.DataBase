IF OBJECT_ID (N'dbo.SP_EXEC_SAMPLE_REVIEW_COUPON_ISSUE', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_EXEC_SAMPLE_REVIEW_COUPON_ISSUE
GO
...
GO
CREATE PROCEDURE [dbo].[SP_EXEC_SAMPLE_REVIEW_COUPON_ISSUE]   
       @P_IDX INT,
       @P_VIEW INT,
       @P_STATUS INT,   
 ...
 BEGIN  
    ...
    */  
     ...
     */  
	 ...
	 /* 비회원 -> 회원 아이디 매칭 */
	 IF CHARINDEX('@', @UID) > 0 AND @USER_ID <> ''
	 BEGIN
		UPDATE S4_EVENT_REVIEW
		   SET ER_UserId = @USER_ID
		 WHERE ER_IDX = @P_IDX
	 END
    ...
    DECLARE @BestCommentCnt INT

	SELECT @BestCommentCnt = COUNT(*) 
			...
			BEGIN  
				...
				BEGIN  
					...
					BEGIN  
						...
						SELECT @COUPON_DETAIL_SEQ_1 = COUPON_DETAIL_SEQ
						  FROM COUPON_DETAIL  
						 ...
						 WHERE UID = @USER_ID  
						   AND COUPON_DETAIL_SEQ IN ( 
							...
							SELECT COUPON_DETAIL_SEQ
						      FROM COUPON_DETAIL 
							 WHERE COUPON_CODE IN ('7B10-5AF7-43D6-B43A', '9693-79AF-4F00-8967', '7F2C-DFEB-4A47-8F00', '3D8F-C9DD-4B3C-981F')
						)      
