IF OBJECT_ID (N'dbo.up_select_ranking_category_product_list_search', N'P') IS NOT NULL DROP PROCEDURE dbo.up_select_ranking_category_product_list_search
GO
...
-- =============================================        
CREATE PROCEDURE [dbo].[up_select_ranking_category_product_list_search]      
...
BEGIN      
 ...
 BEGIN      
  ...
  -- Count Query 시작 --      
  SELECT top 1 COUNT(A.Card_Seq) AS CNT         
 ...
 BEGIN      
  ...
  WHERE 1 = 1      
    ...
    AND E.Company_Seq = @company_seq      
    AND (J.CardKind_Seq in  (1,6) OR (ISNULL(@keyword, '') <> '' AND J.CardKind_Seq = 3)) -- (청첩장, 답례장) OR (검색어 있을때만 감사장 포함)          
          AND (B.Card_Name LIKE '%' + ISNULL(@keyword, '') + '%' OR B.Card_Code LIKE '%' + ISNULL(@keyword, '') + '%' OR  Replace(Replace(B.Card_Name,' ',''),CHAR(10),'') LIKE '%' + ISNULL(@keyword, '') + '%' OR Replace(Replace(B.Card_Code,' ',''),CHAR(10
),'') LIKE '%' + ISNULL(@keyword, '') + '%')            
    AND H.IsQuick = 1      
  -- Count Query 끝 --      
