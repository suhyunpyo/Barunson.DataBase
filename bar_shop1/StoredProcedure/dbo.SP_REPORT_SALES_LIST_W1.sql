IF OBJECT_ID (N'dbo.SP_REPORT_SALES_LIST_W1', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_REPORT_SALES_LIST_W1
GO
...
CREATE PROCEDURE [dbo].[SP_REPORT_SALES_LIST_W1]
...
BEGIN
	...
	SELECT	
		...
		,	SCV.ERP_Code
		,	SCV.Old_Code
		,	ISNULL(CO.unit_price, 0) AS Card_price
        ...
        ,   (ROUND((ISNULL(CO.unit_price, 0) * ((100 - ISNULL(CO.Discount_Rate, 0)) / 100)) + 0.4, 0) * ISNULL(CO.Order_Count, 0)) +
            ISNULL(Env_Option_Price         , 0) +
            ISNULL(CO.Jebon_Price			, 0) +
            ISNULL(CO.Option_Price			, 0) +
            ISNULL(CO.Mini_Price			, 0) +
	...
	FROM    Custom_Order AS CO
		...
		LEFT JOIN Custom_Order_Item AS COI2
			ON CO.Order_Seq = COI2.Order_Seq
				AND COI2.item_count > 0
				AND COI2.item_type = 'C'
	...
	ELSE IF @P_List_Type = 'List_Excel'
		BEGIN
			...
			SELECT	DISTINCT
				...
				,	CASE 
                        WHEN SB.Member_ID IS NULL THEN 'N' 
                        ELSE 'Y' 
                    END AS SampleBook_Order_YN
			...
			FROM	#T AS T
                ...
                ( 
                    ...
                    FROM	Custom_Sample_Order AS CSO
					    INNER JOIN	#T AS T_SUB 
                            ON CSO.Member_ID = T_SUB.Member_ID
                    WHERE	1 = 1
                        AND		CSO.Status_Seq = 12
					    AND		CSO.Member_ID IS NOT NULL
					    AND		CSO.Member_ID <> ''
					    AND		CSO.Request_Date >= DATEADD(YEAR, -1, T_SUB.Order_Date)
                    GROUP BY 
                            CSO.Member_ID
                ) AS CSO 
                    ON CSO.Member_ID = T.Member_ID
                INNER JOIN	S2_CardView AS SCV 
                ...
                (
                    SELECT 
                          member_id
                        , CASE WHEN pg_shopid = 'bhands_c' THEN 'PC' WHEN pg_shopid = 'bhands_cm' THEN 'Mobile' ELSE '' END AS Inflow_Route_SampleBook
