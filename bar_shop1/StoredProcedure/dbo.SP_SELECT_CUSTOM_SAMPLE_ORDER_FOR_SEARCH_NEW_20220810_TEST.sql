IF OBJECT_ID (N'dbo.SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_NEW_20220810_TEST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_NEW_20220810_TEST
GO
...
-- EXEC SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_NEW_20220810_TEST   ''  , ''  , ' SA , SB ,  ST , SS , B , C , H , U , D , Q , P , SG , X , XB , G , SD , BM , '  ,  0 ,  0 , '2022-10-10'  , '00'  , '2022-10-17'  , '23'  , ''  , ''  , ''  ,  0 , ''  ,  0 
...
CREATE PROCEDURE [dbo].[SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_NEW_20220810_TEST]
...
@P_MEMBER_TYPE = 0
	...
	CST.SAMPLE_ORDER_SEQ,
		...
		-- 20221017 상품유형 추가
		--, PRODUCT_KIND = CST.SAMPLE_ORDER_SEQ
		 --STUFF(( 
			--		select C.order_type
			--		--SELECT '/' + CASE	WHEN C.order_type = 0 THEN '청첩_초대장'
			--		--					WHEN C.order_type = 1 THEN '청첩장'
			--		--					WHEN C.order_type = 2 THEN '감사장'
			--		--					WHEN C.order_type = 3 THEN '초대장'
			--		--					WHEN C.order_type = 4 THEN '시즌카드'
			--		--					WHEN C.order_type = 5 THEN '미니청첩장'
			--		--					WHEN C.order_type = 6 THEN '포토청첩장'
			--		--					WHEN C.order_type = 7 THEN '이니셜청첩장'
			--		--					WHEN C.order_type = 7 THEN '이니셜청첩장'
			--		--					WHEN C.order_type = 8 THEN '포토미니청첩장'
			--		--					WHEN C.order_type = 9 THEN '돌포토초대장'
			--		--					WHEN C.order_type = 10 THEN '돌덕담카드'
			--		--					WHEN C.order_type = 11 THEN '돌포스터'
			--		--					WHEN C.order_type = 12 THEN '식순지'
			--		--					WHEN C.order_type = 13 THEN '디지털감사장'
			--		--					ELSE '기타'
			--		--			END
			--			FROM CUSTOM_SAMPLE_ORDER_ITEM A INNER JOIN 
			--					S2_CARDVIEW B ON A.CARD_SEQ = B.CARD_SEQ INNER JOIN 
			--					custom_order C on  A.CARD_SEQ = C.CARD_SEQ
			--			WHERE A.SAMPLE_ORDER_SEQ = CST.SAMPLE_ORDER_SEQ/*2105126 CST.SAMPLE_ORDER_SEQ*/ AND SORT IS NOT NULL
			--			and CST.SAMPLE_ORDER_SEQ is not null
			...
			--						SELECT distinct 
			--							CASE	WHEN B.IsLaser <> '0' THEN '레이저|'
			--							else '' end ,
			--							CASE	WHEN B.IsMasterDigital = '1' THEN '디지털|'
			--							else '' end,
			--							CASE	WHEN substring(LTRIM(B.printmethod),3,1) = '1' THEN '형압|'
			--							else '' end,
			--							CASE	WHEN substring(B.printmethod, 1,1)  <> '0'  THEN '박|'
			--							else '' end
									

			--						FROM CUSTOM_SAMPLE_ORDER_ITEM A INNER JOIN 
			--							 s2_cardoption B ON A.CARD_SEQ = B.CARD_SEQ
		...
		디지털여부 =   CASE WHEN CHARINDEX('EXCEL_WITH_ITEM', @P_LIST_TYPE) > 0 --주문의 아이템개수별
									...
									(
										...
										SELECT 디지털 =  CASE	WHEN B.ISMASTERDIGITAL = '1' THEN '1'
																		ELSE '0' END 
																FROM	CUSTOM_SAMPLE_ORDER_ITEM A INNER JOIN 
																		S2_CARDOPTION B ON A.CARD_SEQ = B.CARD_SEQ
																WHERE SAMPLE_ORDER_SEQ = CST.SAMPLE_ORDER_SEQ AND   
									...
									ELSE (  --주문건별
											...
											SELECT	CASE	WHEN CHARINDEX('Y', TB.디지털) > 0 THEN '1'
													ELSE '0' END ISMASTERDIGITAL
											FROM (
													SELECT 디지털 =  STUFF(
																			(	SELECT ',' + CASE	WHEN B.ISMASTERDIGITAL = '1' THEN 'Y'
																							ELSE 'N' END 
																				FROM CUSTOM_SAMPLE_ORDER_ITEM A INNER JOIN 
																					 S2_CARDOPTION B ON A.CARD_SEQ = B.CARD_SEQ
																				WHERE SAMPLE_ORDER_SEQ = CST.SAMPLE_ORDER_SEQ
