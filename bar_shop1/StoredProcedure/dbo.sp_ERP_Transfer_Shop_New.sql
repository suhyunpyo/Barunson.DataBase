IF OBJECT_ID (N'dbo.sp_ERP_Transfer_Shop_New', N'P') IS NOT NULL DROP PROCEDURE dbo.sp_ERP_Transfer_Shop_New
GO
...
GO
CREATE PROCEDURE [dbo].[sp_ERP_Transfer_Shop_New]  
 @SDate as char(8)  
 , @EDate as char(8)  
...
--EXEC sp_ERP_Transfer_Shop_New '20220823','20220905'  
...
Set @fromDate = COALESCE(@SDate, CONVERT(date, getdate()-2));
Set @toDate = COALESCE(@EDate, CONVERT(date, getdate()-1));
...
WHERE A.status_Seq = 15  -- 배송완료  
 and A.src_send_date BETWEEN @fromDate AND @toDate  
 and A.Order_date >= '20160101'  
...
WHERE  ISNULL(A.Card_Div, '') NOT IN ('C05', 'C13') --사은품 제거  
 ...
 AND CARD_CODE <> 'YM15' --정광수차장 요청 20120313  
 AND CARD_CODE <> '1_misty' --http://redmine.barunson.com/redmine/issues/5742  
 AND CARD_CODE <> 'flower_misty'  

 AND CARD_CODE <> 'seal_flower_p_C1'  
 AND CARD_CODE <> 'sealing_dear_G_C1'  
 AND CARD_CODE <> 'seal_leaf_w_C1'  
 AND CARD_CODE <> 'seal_square_NV_C1'  
 AND CARD_CODE <> 'seal_square_BR_C1'  
 AND CARD_CODE <> 'seal_flower_p_C'  
 AND CARD_CODE <> 'sealing_dear_G_C'  
 AND CARD_CODE <> 'seal_leaf_w_C'  
 AND CARD_CODE <> 'seal_square_NV_C'  
 AND CARD_CODE <> 'seal_square_BR_C'  
...
--전자상거래  
SELECT 'SB' AS sales_gubun, '1450071' AS h_comcode, 'N110' AS h_partCode, '030801' AS h_staffcode, '511' AS h_cost, 'Y' AS PGCheck, 'SB' AS DeptGubun , '5001' AS Company_seq UNION ALL  --바른손카드  
SELECT 'SS' AS sales_gubun, '1450073' AS h_comcode, 'N130' AS h_partCode, '030801' AS h_staffcode, '513' AS h_cost, 'Y' AS PGCheck, 'SS' AS DeptGubun , '5003' AS Company_seq UNION ALL  --프리미어페이퍼  
SELECT 'ST' AS sales_gubun, '2011021' AS h_comcode, 'N140' AS h_partCode, '030801' AS h_staffcode, '514' AS h_cost, 'Y' AS PGCheck, 'ST' AS DeptGubun , '5007' AS Company_seq UNION ALL  --더카드  
  
  
--제휴영업 20220205 수정
SELECT 'SA' AS sales_gubun, '2011022' AS h_comcode, '390' AS h_partCode, '030801' AS h_staffcode, '148' AS h_cost, 'Y' AS PGCheck, 'BR' AS DeptGubun , '5006' AS Company_seq UNION ALL  --비핸즈카드  
SELECT 'C' AS sales_gubun,  '2012664' AS h_comcode, '390' AS h_partCode, '030801' AS h_staffcode, '148' AS h_cost, 'N' AS PGCheck, 'BR' AS DeptGubun , '5780' AS Company_seq UNION ALL  --이지웰   
...
WHERE A.delivery_Date BETWEEN @fromDate AND @toDate  
 ...
 JOIN (  
    ...
    FROM #custom_sample_order a   
    JOIN custom_sample_order_item b ON a.sample_order_seq = b.sample_order_seq  
    JOIN #SampleCardMaster c ON b.card_seq = c.card_seq  
    JOIN #SampleCardMaster d ON c.cont_seq = d.card_seq  
    WHERE a.delivery_Date between @fromDate and @toDate 
	and c.cont_seq is not null 
	and c.cont_seq <> '0'  
    ...
    JOIN custom_sample_order_item b ON a.sample_order_seq = b.sample_order_seq  
    JOIN #SampleCardMaster c ON b.card_seq = c.card_seq  
    JOIN #SampleCardMaster d ON c.acc_seq = d.card_seq  
    WHERE a.delivery_Date between @fromDate and @toDate 
	and c.acc_seq is not null 
	and c.acc_seq <> '0'  
    GROUP BY a.sample_order_seq, d.card_seq,a.settle_price  
    ...
    JOIN custom_sample_order_item b ON a.sample_order_seq = b.sample_order_seq  
    JOIN #SampleCardMaster c ON b.card_seq = c.card_seq  
    JOIN #SampleCardMaster d  ON c.acc_seq2 = d.card_seq  
    WHERE a.delivery_Date between @fromDate and @toDate 
	and c.acc_seq2 is not null 
	and c.acc_seq2 <> '0'  
    GROUP BY a.sample_order_seq, d.card_seq,a.settle_price  
    ...
    JOIN custom_sample_order_item b ON a.sample_order_seq = b.sample_order_seq  
    JOIN #SampleCardMaster c ON b.card_seq = c.card_seq  
    JOIN #SampleCardMaster d  ON c.MapCard_Seq = d.card_seq  
    WHERE a.delivery_Date between @fromDate and @toDate  
     and c.MapCard_Seq is not null 
	 and c.MapCard_Seq <> 0  
    GROUP BY a.sample_order_seq, d.card_seq,a.settle_price  
    ...
    WHERE delivery_Date between @fromDate and @toDate   
      
   )  b ON a.sample_order_seq = b.sample_order_seq  
  
 JOIN company c ON a.company_seq = c.company_seq  
 JOIN #SampleCardMaster d ON b.card_seq = d.card_seq  
 LEFT JOIN #sales_gubunTemp O ON a.sales_gubun = O.sales_gubun  
...
WHERE A.delivery_Date BETWEEN @fromDate AND @toDate  
 ...
 JOIN (  
   ...
   --AND a.order_type NOT IN ('AA','BB','CC','DD','EE','FF','GG','HH','II','JJ','KK','LL','MM','NN','OO','PP','QQ','RR','SS','TT','UU','XX','YY','ZA','ZB','ZC','ZD','ZE','ZF','ZG') --답례품 제외  
   AND a.order_type NOT IN (SELECT code FROM bar_shop1.dbo.manage_code WHERE code_type ='etcprod' AND use_yorn = 'Y' AND LEFT(code, 1) NOT IN ('V') and parent_id = 0) --답례품 제외  
   ...
   WHERE  b.order_count > 0  
      --AND a.order_type IN ('AA','BB','CC','DD','EE','FF','GG','HH','II','JJ','KK','LL','MM','NN','OO','PP','QQ','RR','SS','TT','UU','XX','YY','ZA','ZB','ZC','ZD','ZE','ZF','ZG') -- 답례품만   
      AND a.order_type IN (SELECT code FROM bar_shop1.dbo.manage_code WHERE code_type ='etcprod' AND use_yorn = 'Y' AND LEFT(code, 1) NOT IN ('V') and parent_id = 0) -- 답례품만   
