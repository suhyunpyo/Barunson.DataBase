IF OBJECT_ID (N'dbo.SP_COUPON_CANCEL', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_COUPON_CANCEL
GO
...
/*
	...
	EXEC SP_COUPON_CANCEL 2920831, 's4guest';
*/
CREATE PROCEDURE [dbo].[SP_COUPON_CANCEL]
		@ORDER_SEQ					AS INT
	,	@UID						AS VARCHAR(100)
AS
BEGIN
	...
	ELSE
		BEGIN
			...
			IF @SALES_GUBUN = 'B' OR @SALES_GUBUN = 'H' OR @SALES_GUBUN = 'C'
				BEGIN
					/* 중복쿠폰만 원복 */
					SELECT @ADDITION_COUPONSEQ = ISNULL(ADDITION_COUPONSEQ,'') , @COMPANY_SEQ = COMPANY_SEQ 
					FROM CUSTOM_ORDER 
