IF OBJECT_ID (N'dbo.SP_T_USER_ORDER_AUTO_COUPON_PUBLISH_VER2', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_T_USER_ORDER_AUTO_COUPON_PUBLISH_VER2
GO
...
GO
CREATE PROCEDURE [dbo].[SP_T_USER_ORDER_AUTO_COUPON_PUBLISH_VER2]   
/***************************************************************  
작성자 : 표수현  
작성일 : 2020-02-15  
DESCRIPTION : 바비더프몰 결제완료한 고객에게 쿠폰 자동 발급 
...
AS    
 ...
 FROM (
		SELECT	A.COUPON_ID ,
				만료일 = CASE A.PERIOD_METHOD_CODE 
							...
							(
								...
								FROM BARUNSON.DBO.TB_COMMON_CODE 
		...
		FROM BARUNSON.DBO.TB_COUPON A INNER JOIN  BARUNSON.DBO.TB_COMMON_CODE B ON A.PUBLISH_METHOD_CODE = B.CODE
...
--SELECT COUPON_ID, @USER_ID, 'N', NULL, 만료일, NULL, @USER_ID, GETDATE(), '',@USER_ID, GETDATE(), ''
 --FROM #TEMP
	...
	-- 자동발급설정된 쿠폰이 존재하고, 회원일때 
	IF EXISTS (SELECT COUPON_ID FROM #TEMP) AND 
	   @MEMBERYN > 0 BEGIN 
		
		--BEGIN TRAN 
		
		--INSERT BARUNSON.DBO.TB_COUPON_PUBLISH (COUPON_ID, USER_ID, USE_YN ,USE_DATETIME, EXPIRATION_DATE, RETRIEVE_DATETIME ,REGIST_USER_ID, REGIST_DATETIME, REGIST_IP,
