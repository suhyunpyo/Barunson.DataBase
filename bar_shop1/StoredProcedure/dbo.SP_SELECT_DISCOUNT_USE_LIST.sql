IF OBJECT_ID (N'dbo.SP_SELECT_DISCOUNT_USE_LIST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_SELECT_DISCOUNT_USE_LIST
GO
...
GO
/*EXEC SP_SELECT_DISCOUNT_USE_LIST 126, 100, 1, 'SEQ', 'DESC'*/

CREATE PROCEDURE [dbo].[SP_SELECT_DISCOUNT_USE_LIST]
				...
				FROM	(
							SELECT	  SC.Card_Name as CardName
									...
									, CASE WHEN Company_Seq = 5006 THEN '비핸즈카드' 
                                           ...
                                           WHEN Company_Seq = 5008 THEN '아웃바운드'
                                           ELSE '해외영업상품관리'
									  END SaleSite
									, IsDisplay
