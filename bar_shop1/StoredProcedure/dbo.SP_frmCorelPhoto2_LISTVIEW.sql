IF OBJECT_ID (N'dbo.SP_frmCorelPhoto2_LISTVIEW', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_frmCorelPhoto2_LISTVIEW
GO
...
GO
/*
EXEC SP_frmCorelPhoto2_LISTVIEW '', '', '3', '0', '0', '2021-08-17', '2021-08-17', 0, 0, '2', '20210810', '20210817', '0', '', 10000

EXEC SP_frmCorelPhoto2_LISTVIEW '3143978', '', '0', '0', '0', '2021-03-30', '2021-04-30', 0, 0, '3', '20210430', '20210430', '0', '', 10000
*/
CREATE PROCEDURE [dbo].[SP_frmCorelPhoto2_LISTVIEW]
...
FROM custom_order_plist CP with(nolock) 
JOIN S2_Card CD with(nolock) ON CP.card_seq = CD.Card_Seq 
JOIN CARD_COREL CC with(nolock) ON CD.Card_Code = CC.Card_Code AND ISNULL(CC.WEPOD_YORN, 'N') = 'Y'
...
FROM custom_order_plist CP with(nolock) 
JOIN S2_Card CD with(nolock) ON CP.card_seq = CD.Card_Seq 
JOIN HardCodingList CC with(nolock) ON CD.Card_Code = CC.HardCode AND HardID='DigitalCardCode'
WHERE CP.print_type IN ( 'P' , 'I' ) 
	AND @ORDER_TYPE IN ('12', '14' )
...
--포켓카드중에 내부디지털 작업해야 할 주문건 (2021-08-13 수정)
SELECT DISTINCT order_seq 
INTO #POCKET_SUB_CARD
FROM (select * from custom_order_plist with(nolock) where print_type='S') CP 
JOIN S2_Card CD with(nolock) ON CP.card_seq = CD.Card_Seq 
JOIN HardCodingList CC with(nolock) ON CD.Card_Code = CC.hardcode 
WHERE @ORDER_TYPE IN ('0', '17' ) AND CC.HardID = 'DigitalCardCode' AND CC.HardUse = 'Y'
	...
	SELECT   A.order_name
		...
		, A.EnvPremium_price
		   ...
		   , proc_date1
	INTO #ResultData
	FROM Custom_Order AS A with(nolock)
	JOIN S2_Card AS B with(nolock) ON A.Card_Seq = B.Card_Seq
	JOIN S2_CardOption AS C with(nolock) ON A.Card_Seq = C.Card_Seq 
	JOIN Card_Corel AS D with(nolock) ON B.Card_Code = D.Card_Code 
	LEFT JOIN Custom_Order_Chasu AS E with(nolock) ON A.Order_Seq = E.Order_Seq 
	-- LEFT JOIN (select order_seq, count(order_seq) cnt from custom_order_plist with(nolock) where  print_type <> 'E' group by order_seq ) T ON A.order_seq =  T.order_seq 
	...
	WHERE 1 = 1 
		...
		AND ( 
			...
			OR	( @ORDER_TYPE = '3' 
					AND 
					(
					(( A.order_type = '7' OR C.isLetterPress = '1' OR C.PrintMethod <> '000' )
						AND B.card_code NOT IN (SELECT HardCode FROM HardCodingList WHERE HardID = 'SAMSUNG_GOLDFOIL' AND HardUse = 'Y')
						AND B.card_code NOT IN (SELECT HardCode FROM HardCodingList WHERE HardID = 'HYUNDAI_GOLDFOIL' AND HardUse = 'Y') /* 2021-08-20 추가 */
						AND A.card_seq NOT IN (SELECT card_seq FROM CopperPlateExclude  ) 
						AND (SELECT COUNT(1) FROM custom_order_plist WHERE print_type <> 'E' AND order_seq = A.order_seq) > 0
						-- AND ISNULL(T.cnt,0) > 0
						)
					OR 
					A.envpremium_price > 0
					)
		...
		AND ( 
			...
			OR	( @ORDER_TYPE = '12' AND C.isMasterDigital = '1' and proc_date2 is not null)

			OR  ( @ORDER_TYPE = '12' AND EXISTS ( SELECT Order_seq FROM #DIGITAL_PART where A.order_seq = order_seq  ) )
		...
		AND ( 
				...
				--20220404 고남영님 요청으로 order_type 1-> 청첩장도 추가 
			OR	( @ORDER_TYPE = '14' AND A.order_type in ('1', '6', '7', '13') AND C.isInternalDigital = '1'  and proc_date2 is not null)		-- AND A.order_type = '6' 일단 주석처리.

	
			--OR	( @ORDER_TYPE = '14' AND A.order_type in ('6', '7', '13') AND C.isInternalDigital = '1'  and proc_date2 is not null)		-- AND A.order_type = '6' 일단 주석처리.
		)

		--포켓카드(디지털) 검색시
		AND ( 
				( @bWhereBypass = 'Y' OR @ORDER_TYPE <> '17' )	--조건통과
			OR	( A.ORDER_SEQ in (SELECT ORDER_SEQ FROM #POCKET_SUB_CARD) and proc_date2 is not null)
		)
		...
		AND ( 
			...
			OR	( @ORDER_TYPE = '0' AND A.envpremium_price > 0 )
			...
			OR	( @ORDER_TYPE = '0' AND A.order_type in ('6', '7', '13') AND C.isInternalDigital = '1'  )
		)
		

SELECT TOP (@PageSize) A.*, B.pseq 
FROM #ResultData A
LEFT JOIN Custom_Order_Chasu B with(nolock) ON A.order_seq = B.order_seq
WHERE A.order_seq IN ( SELECT DISTINCT A.order_seq FROM #ResultData A JOIN custom_order_plist B with(nolock) ON A.order_seq = B.order_seq AND B.isNotPrint = 0 )
