IF OBJECT_ID (N'dbo.SP_PP_MAIN_RANKING_LIST_TEST', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_PP_MAIN_RANKING_LIST_TEST
GO
...
-- =======================================================================================
CREATE PROCEDURE [dbo].[SP_PP_MAIN_RANKING_LIST_TEST] 
...
BEGIN
	...
	IF @brand = 'I' OR @brand = 'N' --I : 인기상품 , N : 신상품
		...
		BEGIN
			...
			--SELECT  
			--	row_number()over(order by a.rk_idx) rm,count(*)over() cnt,
			--	card_seq,isbest, isnew, isextra,  card_code, rk_title as card_name, cardset_price,card_content,	discount_rate, cardimage_filename,	issample, 
			--	cardsale_price card_sale_price, card_text_premier, regdate,isfsc,  
			--	replace( convert( varchar, convert(money , (round(cardset_price * ((100 - d.discount_rate) / 100) , 0)) * @ordernum), 1 ), '.00', '' )  card_price , wishcard	,'M' gb
			--FROM S4_RANKING_SORT_TABLE AS A LEFT OUTER JOIN (SELECT DISTINCT B.CARD_SEQ, C.COMPANY_SEQ,
			...
			--														,ROW_NUMBER()OVER(PARTITION BY B.CARD_SEQ ORDER BY G.CARDIMAGE_FILENAME ) S_RM	
			--														,ISNULL((SELECT TOP 1  'Y' FROM S2_WISHCARD WHERE CARD_SEQ = B.CARD_SEQ AND UID=@UID ) , 'N') WISHCARD			
			...
			--													WHERE CARDIMAGE_WSIZE = @IMAGE_WSIZE
			--													AND CARDIMAGE_HSIZE = @IMAGE_HSIZE
			--													AND CARDIMAGE_DIV ='E' 
			--													AND G.COMPANY_SEQ = @COMPANY_SEQ
			--													AND C.COMPANY_SEQ = @COMPANY_SEQ
			--													AND J.MINCOUNT = @RE_MINCOUNT
			...
			SET @strQuery = @strQuery + 'select rm , cnt ,card_seq,isbest, isnew, isextra,  card_code,card_name , cardset_price,card_content '+char(13) + char(10)
			SET @strQuery = @strQuery + '		,	discount_rate, cardimage_filename,	issample,  '+char(13) + char(10)
			SET @strQuery = @strQuery + '	   card_sale_price, card_text_premier, regdate,isfsc,card_price , wishcard, sampleCnt	,gb		 '+char(13) + char(10)
			SET @strQuery = @strQuery + 'from ( '+char(13) + char(10)
	...
	ELSE	--H : 고가상품 , L : 저가상품
		BEGIN
			...
			SET @strQuery = @strQuery + '				,card_text_premier	,regdate	,isFSC  ,Wishcard, sampleCnt ,replace( convert( VARCHAR, convert(money , (Round(cardset_price * ((100 - discount_rate) / 100) , 0)) * @IN_ordernum), 1 ), ''.00'', '''' )  card_price' +char(13) + char(10)
			SET @strQuery = @strQuery + '		from (' +char(13) + char(10)
			...
			--SET @strQuery = @strQuery + ' '
			--SET @strQuery = @strQuery + ' select top 1 ''P'' gb '
			--SET @strQuery = @strQuery + '	 , 8 RE_RM_NO '
			--SET @strQuery = @strQuery + '	 , seq card_seq ,'''' isbest ,'''' isnew	, '''' isextra ,'''' card_code	,'''' card_name,0 cardset_price,0 discount_rate	,'''' cardimage_filename	,'''' issample ,0 card_sale_price'
			--SET @strQuery = @strQuery + '		,'''' card_text_premier,'''' regdate	,'''' isFSC ,'''' card_price ,''N'' Wishcard'
			--SET @strQuery = @strQuery + '	 , title ,  thumb_img link_url_img , ''''link_url, '''' link_target, '''' view_div, '''' jehu_view_div, reg_date ,'''' md_content , row_number()over(order by reg_date desc  ) aa '
			--SET @strQuery = @strQuery + '	   --,'''' card_content' +char(13) + char(10)
			--SET @strQuery = @strQuery + ' from S4_Portfolio'
			--SET @strQuery = @strQuery + ' where company_seq = @IN_company_seq'
			--SET @strQuery = @strQuery + ' and section_gubun = ''Portfolio'''
			SET @strQuery = @strQuery + ' 			) A' +char(13) + char(10)
			SET @strQuery = @strQuery + ' 	) A' +char(13) + char(10)
