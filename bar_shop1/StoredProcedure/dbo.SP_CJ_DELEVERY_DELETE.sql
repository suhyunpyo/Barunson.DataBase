IF OBJECT_ID (N'dbo.SP_CJ_DELEVERY_DELETE', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_CJ_DELEVERY_DELETE
GO
...
--				--proc_CloseSample  
-- =============================================
CREATE PROCEDURE [dbo].[SP_CJ_DELEVERY_DELETE]
		...
		BEGIN TRAN TR_DELIVERY_INFO
				...
				BEGIN		
						...
						WHILE CharIndex(',', @STR_SEQ, 0) > 0
							BEGIN
								...
								WHERE ISUSE='2' AND API_YN IS NULL
								ORDER BY CODESEQ
			...
			ELSE	-- proc_DelCodeAddPacking / proc_DelCodeAdd /proc_CloseCopyBaesong /
				BEGIN
					SELECT TOP 1 @DEL_CODE =  DELCODE_SEQ ,	@CODE = CODE , @CODESEQ = CODESEQ
					FROM CJ_DELCODE with (HOLDLOCK, rowlock, xlock)
					WHERE ISUSE='0' AND API_YN IS NULL 
					ORDER BY CODESEQ
