IF OBJECT_ID (N'dbo.SP_SELECT_CARD_BY_SAMPLE_TOTAL_LIST_HG', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_SELECT_CARD_BY_SAMPLE_TOTAL_LIST_HG
GO
...
EXEC SP_SELECT_CARD_BY_SAMPLE_TOTAL_LIST_HG '', '2018-01-01', '2018-01-15', 'B'

***/
CREATE Procedure [dbo].[SP_SELECT_CARD_BY_SAMPLE_TOTAL_LIST_HG]  
...
BEGIN
	...
	--토탈건수
	SELECT max(statsname) as StatsName, SUM(BRC) AS BarunsonCnt, SUM(NBRC) AS NBarunsonCnt, SUM(BHC) AS  BhandsCnt, SUM(THC) AS ThecardCnt, SUM(JHC) AS MallCnt, SUM(PRC) AS PremierCnt, SUM(BRC+THC+BHC+JHC+PRC+NBRC) AS SumTotalCnt
	FROM
	(
		SELECT
		 '총샘플수량' as statsName
		...
		FROM S2_CARD SC
			LEFT JOIN ( 
						...
						(
							...
							, SUM(CASE WHEN SALES_GUBUN IN ('ST') THEN Cnt ELSE 0 END ) AS thecard 
	...
	(
		...
		, SUM(CASE WHEN SALES_GUBUN IN ('ST') THEN 1 ELSE 0 END ) AS thecardCnt
