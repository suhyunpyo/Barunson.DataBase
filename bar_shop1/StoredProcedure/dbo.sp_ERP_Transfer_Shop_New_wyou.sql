IF OBJECT_ID (N'dbo.sp_ERP_Transfer_Shop_New_wyou', N'P') IS NOT NULL DROP PROCEDURE dbo.sp_ERP_Transfer_Shop_New_wyou
GO
...
CREATE PROCEDURE [dbo].[sp_ERP_Transfer_Shop_New_wyou]
 @SDate as char(8)
 , @EDate as char(8)
...
SELECT 'SS' AS sales_gubun, '1450073' AS h_comcode, 'N130' AS h_partCode, '110201' AS h_staffcode, '513' AS h_cost, 'Y' AS PGCheck, 'SS' AS DeptGubun , '5003' AS Company_seq UNION ALL		--프리미어페이퍼
...
--###############################################################################################################################################################
	...
	UNION 
	SELECT order_seq, card_seq,  'QICKDELIVERY', 'QICKDELIVERY', 1, 30000 AS delivery_price
...
SELECT 'BK10' AS h_biz, 'SO' AS h_gubun, Convert(char(8),a.src_send_date,112) AS h_date
	--, '270', '270'
		...
		--, Case  When c.company_seq in (232,1137,1250,2235) Then --디지털플라자, 워커힐, G마켓 (후불결제), 전자랜드
		--			Case When (b.item_Type ='F'  and b.item_price > 0 )         --후불결제 업체라도 식권, 엠보인쇄 등 옵션이나 추가주문건은 PG를 타서 결제함 (주문번호 앞에 'IC' 붙힘)
		--					or (a.up_order_seq is not null and a.settle_price > 0) 
...
-- 배송비는 안올림
	...
	SELECT 
		...
		CASE WHEN ISNULL(O.h_comcode, '') = '1489998' THEN ISNULL(C.ERP_PartCode, ISNULL(O.h_partCode, '')) ELSE O.h_partCode  END,  --1489998	BA_제휴사 일때만 C.ERP_PartCode 이걸루 수정 20111120 이상민
		CASE WHEN ISNULL(C.ERP_PartCode, '') = '366' THEN '080401' ELSE O.h_staffcode  END,  --366	웨딩(비직영) 일때 080401배민영으로 수정 20141120  (구: O.h_staffcode )
		h_sonik		= '110',
...
WHERE Convert(char(8), a.delivery_Date,112) BETWEEN @SDate AND @EDate
	...
	SELECT 
		...
		b_OrderNum	=  Case  When b.Card_ErpCode = 'BSI010' Then (b.order_count/50)*6  
							When b.Card_ErpCode IN ( 'BSI050', 'BSI051', 'BSI052', 'BSI053' ) AND b.order_count < 100 Then b.order_count * 25  --스티커 'BSI050', 'BSI051', 'BSI052', 'BSI053' 1판에25장으로 계산해서 입력(정광수차장요청)
						Else b.order_count End,	
		
		--**************************************************************************************************************
		--   * 아이템별 금액 셋팅
		--**************************************************************************************************************
		b_unitPrice	 =  Case When b.Card_ErpCode = 'TBB' Then b.item_Price	Else e.c_sobi End,
		b_offerPrice =  Case When b.Card_ErpCode = 'TBB' Then Round(b.item_Price/1.1,0)	Else Round(e.c_sobi*b.order_count/1.1,0) End,	
		b_superTax	 =  Case When b.Card_ErpCode = 'TBB' Then b.item_Price - Round(b.item_Price/1.1,0)	Else (e.c_sobi*b.order_count) - Round(e.c_sobi*b.order_count/1.1,0) End,	
		b_sumPrice	 =  Case When b.Card_ErpCode = 'TBB' Then b.item_Price	Else e.c_sobi*b.order_count End,	
		b_memo		= null,
		reg_date	= getdate(),		
	...
	JOIN (
			...
			FROM (
					...
					WHERE  Convert(char(8),delivery_date,112) between @SDate AND @EDate
						and a.status_seq = 12 
						and a.order_type in ('F','P', 'C', 'G', 'S', 'D','K', 'B', 'R', 'Q', 'J', 'H', 'N', 'E')  --속지봉투:E 추가 20161102 이상민
			...
			WHERE Convert(char(8),delivery_date,112) between @SDate AND @EDate
				...
				and status_seq = 12 
				and order_type in ('F','P', 'C', 'G', 'S', 'D','K', 'B', 'R', 'Q', 'J', 'H', 'N', 'E')
	...
	WHERE Convert(char(8),a.delivery_date,112) between @SDate AND @EDate
		and a.status_seq = 12 
		and a.order_type in ('F','P', 'C', 'G', 'S', 'D','K', 'B', 'R', 'Q', 'J', 'H', 'N', 'E')
		and ISNULL(a.sales_gubun, '') NOT IN ( 'D' , 'X')	--20111219 이상민 'X' 추가  시즌카드 연동화면 이용함. http://222.120.91.56:8084/erp_NewReport/season_erp_input.asp
		and A.sales_gubun in ( 'W','T','J','B','S','G','SA', 'ST', 'SB', 'SW', 'SS', 'SP', 'SH', 'H', 'C', 'SG', 'P' ) --비핸즈, 더카드
