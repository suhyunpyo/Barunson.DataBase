IF OBJECT_ID (N'dbo.SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_Test', N'P') IS NOT NULL DROP PROCEDURE dbo.SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_Test
GO
...
GO
create PROCEDURE [dbo].[SP_SELECT_CUSTOM_SAMPLE_ORDER_FOR_SEARCH_Test]
...
BEGIN
	...
	SET @SEARCH_DATE_START_OF_DAY = CAST(@P_SEARCH_START_DATE + ' 00:00:00' AS DATETIME)
	SET @SEARCH_DATE_END_OF_DAY = CAST(@P_SEARCH_END_DATE + ' 23:59:59' AS DATETIME)
	...
	IF @P_SEARCH_DATE_TYPE = 0 
	  Begin
		...
		Select [sample_order_seq] From CUSTOM_SAMPLE_ORDER Where REQUEST_DATE >= @SEARCH_DATE_START_OF_DAY And REQUEST_DATE <= @SEARCH_DATE_END_OF_DAY
	  End
	Else IF @P_SEARCH_DATE_TYPE = 1 
	   Begin
		insert into @sampleorderseqs
		Select [sample_order_seq] From CUSTOM_SAMPLE_ORDER Where SETTLE_DATE >= @SEARCH_DATE_START_OF_DAY And SETTLE_DATE <= @SEARCH_DATE_END_OF_DAY
	  End
	Else IF @P_SEARCH_DATE_TYPE = 2 
	   Begin
		insert into @sampleorderseqs
		Select [sample_order_seq] From CUSTOM_SAMPLE_ORDER Where PREPARE_DATE >= @SEARCH_DATE_START_OF_DAY And PREPARE_DATE <= @SEARCH_DATE_END_OF_DAY
	  End
	Else IF @P_SEARCH_DATE_TYPE = 3 
	   Begin
		insert into @sampleorderseqs
		Select [sample_order_seq] From CUSTOM_SAMPLE_ORDER Where DELIVERY_DATE >= @SEARCH_DATE_START_OF_DAY And DELIVERY_DATE <= @SEARCH_DATE_END_OF_DAY
	  End
