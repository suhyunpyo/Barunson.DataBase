IF OBJECT_ID (N'dbo.proc_CloseCopyBaesong_CJ', N'P') IS NOT NULL DROP PROCEDURE dbo.proc_CloseCopyBaesong_CJ
GO
...
GO
CREATE  Procedure [dbo].[proc_CloseCopyBaesong_CJ]
@order_seq int,
@del_id int
as
begin
 ...
 --   insert into DELIVERY_INFO_DELCODE(order_seq,delivery_id,delivery_code_num,delivery_com) values(@order_seq,@del_id,@del_code, @DELIVERY_COMPANY_SHORT_NAME)
	...
	EXEC [dbo].[SP_CJ_DELEVERY_CJ] 'DELIVERY_INFO_DELCODE|DELIVERY_INFO|', @order_seq, @del_id, @DELIVERY_COMPANY_SHORT_NAME, @DEL_CODE OUTPUT
