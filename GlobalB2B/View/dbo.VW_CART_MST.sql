...
AS
SELECT     CART_SEQ, USER_SEQ, ORDER_SEQ, PROD_SEQ, CART_STATE_CODE, QUANTITY, REG_DATE, PRICE,
                          (SELECT     ISNULL(SUM(PRICE), 0) AS Expr1
                            FROM          dbo.ADDITIONAL_PRICE_MST
                            WHERE      (FOREIGN_SEQ = CM.CART_SEQ) AND (ADD_PRICE_TYPE_CODE = '120001' OR
                                                   ADD_PRICE_TYPE_CODE = '120002')) + PRICE AS CAL_PRICE, UNIT_PRICE
FROM         dbo.CART_MST AS CM
