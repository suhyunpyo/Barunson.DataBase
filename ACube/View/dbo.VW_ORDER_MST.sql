...
AS
SELECT     ORDER_SEQ, ORDER_CODE, ORDER_STATUS_TYPE_CODE, REQUEST_STATUS_TYPE_CODE, REG_DATE, XML_DATA_PATH, MEMO, ERP_INSERT_YORN, 
                      ERP_INSERT_DATE,
                          (SELECT     SUM(PRICE) AS Expr1
                            FROM          dbo.VW_CART_MST AS VCM
                            WHERE      (ORDER_SEQ = OM.ORDER_SEQ)) AS PRICE,
                          (SELECT     SUM(CART_ITEM_PRINT_COUNT) AS Expr1
                            FROM          dbo.VW_CART_MST AS VCM
                            WHERE      (ORDER_SEQ = OM.ORDER_SEQ)) AS CART_ITEM_PRINT_COUNT
FROM         dbo.ORDER_MST AS OM
